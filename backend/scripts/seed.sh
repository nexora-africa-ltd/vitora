#!/usr/bin/env bash
# =============================================================================
# One-time Data Seeding Script for Vitora HMIS
# =============================================================================
# Run AFTER the container is healthy:
#   az containerapp exec -n vitora-api -g vitora-rg --command "bash scripts/seed.sh"
#
# Or locally:
#   export DATABASE_URL="..." DJANGO_SETTINGS_MODULE=hmis.settings.staging
#   bash scripts/seed.sh
# =============================================================================
set -e

echo "==> Seeding Vitora HMIS data..."
...
# Configure Metabase dashboards for embedding (idempotent, requires METABASE_API_KEY)
python manage.py configure_metabase_embedding || echo "⚠ Metabase embedding config skipped (Metabase may not be ready)"
