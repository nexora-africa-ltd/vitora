#!/usr/bin/env bash
# =============================================================================
# Azure Container Apps — Update Environment Variables & Secrets (PRODUCTION)
# =============================================================================
# Production-specific version of azure-update-env.sh.
# Uses production settings, domains, and enforcements.
#
# Prerequisites:
...
#   bash scripts/azure-update-env-production.sh
# =============================================================================
set -euo pipefail
...
if [[ -z "$FQDN" ]]; then
  ...
  echo "       Create the production container app first."
  exit 1
fi
...
echo "==> Updating PRODUCTION container app: $APP_NAME (https://${FQDN})"
...
az containerapp update \
  ...
  --set-env-vars \
    ...
    "SHA_API_BASE_URL=${SHA_API_BASE_URL:-https://api.dha.go.ke}" \
    ...
    "SHA_AGENT=${SHA_AGENT:-}" \
    "SHA_FHIR_BASE_URL=${SHA_FHIR_BASE_URL:-}" \
    "SHA_API_TIMEOUT=${SHA_API_TIMEOUT:-30}" \
    "SHA_FACILITY_FR_CODE=${SHA_FACILITY_FR_CODE:-}" \
    "SHA_TIBERBU_BASE_URL=${SHA_TIBERBU_BASE_URL:-}" \
    ...
    "FACILITY_KRA_PIN=${FACILITY_KRA_PIN:-}" \
    ...
    "MPESA_CALLBACK_URL=https://api.vitora.digital/api/billing/mpesa/callback/" \
    ...
    "SYNC_ENABLED=true" \
    "SYNC_SERVER_URL=${SYNC_SERVER_URL:-${PUBLIC_API_BASE_URL}/api/sync}" \
    ...
    "SENTRY_MAX_BREADCRUMBS=${SENTRY_MAX_BREADCRUMBS:-50}" \
    ...
    "WEBAUTHN_ORIGIN=${WEBAUTHN_ORIGIN:-https://app.vitora.digital}" \
    ...
    "POWERSYNC_JWT_AUDIENCE=${POWERSYNC_JWT_AUDIENCE:-}" \
    "VAPID_PUBLIC_KEY=${VAPID_PUBLIC_KEY:-}" \
    "VAPID_PRIVATE_KEY=secretref:vapid-private-key" \
    "VAPID_CLAIM_EMAIL=${VAPID_CLAIM_EMAIL:-mailto:info@nexora.africa}" \
    "LIS_STANDALONE_MODE=false" \
    "PHARMACY_STANDALONE_MODE=false" \
    "IMAGING_STANDALONE_MODE=false" \
    "INSURANCE_ENABLED=${INSURANCE_ENABLED:-true}" \
