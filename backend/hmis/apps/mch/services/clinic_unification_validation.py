...
def get_unification_metrics() -> dict[str, int]:
    ...
    return {
        "anc_visits_without_clinic_visit": ANCVisit.objects.filter(
            clinic_visit__isnull=True
        ).count(),
        "pnc_visits_without_clinic_visit": PNCVisit.objects.filter(
            clinic_visit__isnull=True
        ).count(),
        "anc_completed_clinic_visits_without_payload": anc_completed_visits.filter(
            anc_visit__isnull=True
        ).count(),
        "pnc_completed_clinic_visits_without_payload": pnc_completed_visits.filter(
...
def _anc_enrollment_mismatch_queryset():
    return (
        ClinicEnrollment.objects.filter(clinic__clinic_type="ANC")
        .annotate(
            canonical_visit_count=Count(
                ...
                filter=Q(
                    ...
                    patient__mch_registrations__anc_visits__clinic_visit__session__clinic_id=models.F(
                        "clinic_id"
                    ),
                ),
                distinct=True,
...
def validate_mch_clinic_links() -> ValidationSummary:
    ...
    for visit in ANCVisit.objects.select_related("registration__mother").filter(
        clinic_visit__isnull=True
    ):
        details.append(
            {
    ...
    for visit in PNCVisit.objects.select_related("registration__mother").filter(
        clinic_visit__isnull=True
    ):
        details.append(
            {
...
def validate_enrollment_attendance_counts() -> ValidationSummary:
    details: list[dict[str, Any]] = []
    enrollments = ClinicEnrollment.objects.filter(clinic__clinic_type="ANC").select_related(
        "clinic", "patient"
    )
    mismatch_count = 0
            ...
            details.append(
                {
                    ...
                    "stored_last_visit_date": (
                        enrollment.last_visit_date.isoformat()
                        if enrollment.last_visit_date
                        else None
                    ),
                    "canonical_last_visit_date": (
                        latest_visit_date.isoformat() if latest_visit_date else None
                    ),
                }
            )
        ...
        details.append(
            {
                ...
                "clinic_visit_encounter_id": (
                    visit.clinic_visit.encounter_id if visit.clinic_visit else None
                ),
            }
        )
