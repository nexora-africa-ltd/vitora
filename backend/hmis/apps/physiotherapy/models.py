"""
Physiotherapy models for Vitora HMIS.

This module contains all physiotherapy-related models including:
- PhysiotherapyTreatmentType: Catalog of treatment types with pricing
- PhysiotherapyOrder: Referral orders for physiotherapy treatment
- PhysiotherapySession: Individual treatment sessions
...
def generate_physio_order_number():
    """
    Generate a unique Physiotherapy Order Number.

    Format: PHYSIO-YYYYMMDD-XXXX
    Where XXXX is a 4-digit sequential number for the day.

    Returns:
        str: A unique physiotherapy order number string
    """
    today = datetime.now().strftime("%Y%m%d")
    prefix = f"PHYSIO-{today}-"

    # Get the PhysiotherapyOrder model via the app registry to avoid circular imports
    PhysiotherapyOrder = apps.get_model("physiotherapy", "PhysiotherapyOrder")

    # Find the highest order number for today
    latest_order = (
        PhysiotherapyOrder.objects.filter(order_number__startswith=prefix)
...
class PhysiotherapyTreatmentType(models.Model):
    """
    Catalog of physiotherapy treatment types.

    Stores information about available physiotherapy treatments including
...
class PhysiotherapyOrder(HistoryMixin, FacilityScopedModel):
    ...
    Represents a physiotherapy referral made by a clinician for a patient,
    tracking treatment plan, scheduling, and progress.
    """
    ...
    clinical_indication = models.TextField(help_text="Clinical reason for physiotherapy referral")
    ...
    treatment_goals = models.TextField(blank=True, help_text="Goals for physiotherapy treatment")
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:
            ...
            if not self.order_number:
                self.order_number = generate_physio_order_number()
        else:
            # Existing instance - prevent order_number modification
            try:
                original = PhysiotherapyOrder.objects.get(pk=self.pk)
                if original.order_number:
                    self.order_number = original.order_number
            except PhysiotherapyOrder.DoesNotExist:
                pass
...
class PhysiotherapySession(models.Model):
    """
    Individual physiotherapy treatment session.

    Records details of each physiotherapy session including
    ...
    pre_pain_score = models.PositiveIntegerField(
        null=True,
        blank=True,
        choices=PAIN_SCALE,
    ...
    # Session details
    interventions = models.TextField(blank=True, help_text="Interventions/treatments performed")
    exercises_performed = models.TextField(
        blank=True,
        help_text="Exercises performed during session",
    )
    modalities_used = models.TextField(
        blank=True,
        help_text="Modalities used (e.g., ultrasound, TENS, heat, ice)",
    )
    patient_response = models.TextField(
        blank=True,
        help_text="Patient's response to treatment",
    )

    # Post-session assessment
    post_pain_score = models.PositiveIntegerField(
        null=True,
        blank=True,
        choices=PAIN_SCALE,
    ...
    # Home program
    home_exercises = models.TextField(blank=True, help_text="Home exercises prescribed")
    home_exercise_instructions = models.TextField(
        blank=True,
        help_text="Instructions for home exercises",
    )
    ...
    def pain_improvement(self):
        """Calculate pain score improvement (negative = improvement)."""
        if self.pre_pain_score is not None and self.post_pain_score is not None:
            return self.post_pain_score - self.pre_pain_score
        return None
