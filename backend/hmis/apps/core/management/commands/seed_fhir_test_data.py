...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"  bundle_id:        ips-{patient.id}")
        self.stdout.write(f"  composition_id:   {patient.id}  (same as patient_id)")
        self.stdout.write(f"  practitioner_id:  {staff_profile.id}")
        self.stdout.write(f"  practitioner_role_id:  {staff_profile.id}")
        self.stdout.write(f"  organization_id:  {clinic.id}")
        self.stdout.write(f"  condition_id:     {diagnosis1.id}")
        self.stdout.write(f"  allergy_intolerance_id:  {allergy.id}")
        self.stdout.write(f"  medication_id:    {medication_drug.id}")
        self.stdout.write(f"  medication_statement_id:  {prescription_item.id}")
        self.stdout.write(f"  observation_results_laboratory_id:  {lab_result.id}")
        self.stdout.write(f"  observation_results_id:             {lab_result.id}")
        self.stdout.write(f"  observation_results_pathology_id:   {lab_result.id}")
        self.stdout.write(f"  observation_results_radiology_id:   {dicom_study.id}")
