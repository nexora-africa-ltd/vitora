...
from hmis.apps.laboratory.services import LabOrderWorkflow, InvalidTransitionError
...
class TestLabOrderWorkflow:
    ...
    def test_valid_transition_ordered_to_collected(
    ...
    def test_valid_transition_collected_to_in_progress(
        self, sample_lab_order, test_user
    ):
    ...
    def test_cancellation_requires_reason(self, sample_lab_order, test_user):
        ...
        updated_order = workflow.transition_to(
            'CANCELLED',
            user=test_user,
            cancellation_reason='Patient declined test'
        )
