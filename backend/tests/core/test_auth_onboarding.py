...
class TestDirectStaffCreationCredentials:
    ...
    def test_create_staff_with_manual_password_does_not_return_temp_password(
        self, admin_client, test_role, test_department
    ):
        """Manual passwords are not echoed back in the response."""
        response = admin_client.post(
            "/api/staff/",
            {
                "username": "manualpass",
                "email": "manual@hospital.co.ke",
                "first_name": "Manual",
                "last_name": "Password",
                "employee_id": "VH-MANUAL-001",
                "role": test_role.id,
                "department": test_department.id,
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert "temp_password" not in response.data
        assert response.data.get("email_sent") is not True

        user = User.objects.get(username="manualpass")
        assert user.check_password("MySecureP@ssw0rd")
        assert user.staff_profile.must_change_password is False

    def test_create_staff_auto_generates_employee_id(
        self, admin_client, test_role, test_department
    ):
        """Blank employee_id is auto-generated by the backend."""
        response = admin_client.post(
            "/api/staff/",
            {
                "username": "autoid",
                "email": "autoid@hospital.co.ke",
                "first_name": "Auto",
                "last_name": "Generated",
                "role": test_role.id,
                "department": test_department.id,
        ...
        assert response.data["employee_id"].startswith("VH-")

    def test_create_staff_generated_password_always_forces_reset(
        self, admin_client, test_role, test_department
    ):
        """Auto-generated passwords must force a reset even if admin tries to bypass."""
        response = admin_client.post(
            "/api/staff/",
            {
                "username": "forcegen",
                "email": "forcegen@hospital.co.ke",
                "first_name": "Force",
                "last_name": "Reset",
                "role": test_role.id,
                "department": test_department.id,
        ...
        user = User.objects.get(username="forcegen")
        assert user.staff_profile.must_change_password is True


# ============================================================================
# Admin Staff Password Reset Tests
# ============================================================================


class TestAdminStaffPasswordReset:
    """Tests for POST /api/staff/{id}/reset-password/."""

    def test_reset_password_generates_temp_password(self, admin_client, test_staff_profile):
        """Admin reset with no password generates a temp password."""
        original_password = test_staff_profile.user.password
        response = admin_client.post(
            f"/api/staff/{test_staff_profile.id}/reset-password/",
            {},
        )
        assert response.status_code == status.HTTP_200_OK
        assert "temp_password" in response.data
        ...
        test_staff_profile.user.refresh_from_db()
        test_staff_profile.refresh_from_db()
        assert test_staff_profile.user.password != original_password
        assert test_staff_profile.user.check_password(response.data["temp_password"])
        assert test_staff_profile.must_change_password is True

    def test_reset_password_with_manual_password(self, admin_client, test_staff_profile):
        """Admin reset with manual password updates it and does not echo it back."""
        response = admin_client.post(
            f"/api/staff/{test_staff_profile.id}/reset-password/",
            {
        ...
        test_staff_profile.user.refresh_from_db()
        assert test_staff_profile.user.check_password("NewSecureP@ssw0rd")
        assert test_staff_profile.must_change_password is False

    def test_reset_password_validates_weak_password(self, admin_client, test_staff_profile):
        """Manual passwords must meet minimum length."""
        response = admin_client.post(
            f"/api/staff/{test_staff_profile.id}/reset-password/",
            {"password": "short"},
        )
    ...
    def test_reset_password_non_admin_forbidden(self, api_client, test_staff_profile):
        """Only admins may reset staff passwords."""
        api_client.force_authenticate(user=test_staff_profile.user)
...
class TestValidatePassword:
    """Tests for POST /api/core/auth/validate-password/ (public)."""

    def test_valid_password_returns_200(self, client):
        response = client.post(
            "/api/core/auth/validate-password/",
            {"password": "StrongP@ssword1!"},
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.data["valid"] is True

    def test_too_short_returns_400_with_errors(self, client):
        response = client.post(
            "/api/core/auth/validate-password/",
            {"password": "short"},
        )
    ...
    def test_common_password_returns_400_with_errors(self, client):
        response = client.post(
            "/api/core/auth/validate-password/",
