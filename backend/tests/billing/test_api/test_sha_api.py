...
class TestSHAAPIPermissions:
    ...
    def test_appeal_claim_requires_appeal_permission(self, authenticated_client, sample_sha_claim):
    ...
    def test_verify_eligibility_requires_permission(self, authenticated_client, sample_sha_member):
...
class TestSHAAPISearch:
    ...
    def test_search_members_by_sha_number(self, sha_client, sample_sha_member):
        ...
        response = sha_client.get(
            f"/api/sha/members/search/?sha_number={sample_sha_member.sha_number}"
        )
    ...
    def test_search_members_by_national_id(self, sha_client, sample_sha_member):
        ...
        response = sha_client.get(
            f"/api/sha/members/search/?national_id={sample_sha_member.national_id}"
        )
...
class TestClaimSubmission:
    ...
    def test_submit_valid_claim(
        self, mock_connectivity, mock_submit_api, sha_client, sample_claim_with_items
    ):
    ...
    @patch("hmis.apps.billing.services.sha_claims.SHAClaimsService._submit_to_sha_api")
    @patch("hmis.apps.billing.services.sha_claims.ConnectivityChecker")
    def test_submit_updates_timestamps(
        self, mock_connectivity, mock_submit_api, sha_client, sample_claim_with_items
    ):
...
class TestClaimsExport:
    ...
    def test_export_claims_excel(self, sha_client, multiple_sha_claims):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (
            "spreadsheet" in response["Content-Type"] or "excel" in response["Content-Type"].lower()
        )
...
# =============================================================================
# Test Class: Claim Submission Connectivity
# =============================================================================


@pytest.mark.django_db
class TestOfflineClaimQueuing:
    """Test live-connectivity requirements for claim submission."""

    def test_claim_submission_fails_when_offline(self, sha_client, sample_claim_with_items):
        """Should fail claim submission when offline."""
        ...
        assert "live SHA connectivity" in str(response.data["error"])

        # Verify claim status not changed
        sample_sha_claim.refresh_from_db()
        assert sample_sha_claim.status == "validated"
    ...
    def test_force_online_fails_when_offline(self, sha_client, sample_claim_with_items):
        ...
        with patch("hmis.apps.billing.services.sha_claims.ConnectivityChecker") as MockChecker:
            ...
            response = sha_client.post(
                f"/api/sha/claims/{sample_sha_claim.id}/submit/", {"force_online": True}
            )

        # Should fail since we're offline
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "live SHA connectivity" in str(response.data.get("error", ""))

    def test_no_sync_queue_entry_created_on_offline_submission(
        self, sha_client, sample_claim_with_items
    ):
        """Should not create SyncQueue entry when offline submission fails."""
    ...
    def test_network_error_returns_submission_failure(self, sha_client, sample_claim_with_items):
        """Should fail claim when network error occurs during submission."""
        import requests
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "Submission failed" in str(response.data.get("error", ""))
...
class TestProcessQueuedClaims:
    ...
    def test_process_queued_claims_success(
        ...
        # Seed a legacy queued claim directly.
        SHAClaimsService()._queue_claim_for_submission(sample_sha_claim, user_with_sha_permissions)
    ...
    def test_process_queued_claims_marks_entry_synced(self, sha_client, sample_claim_with_items):
        ...
        queue_result = SHAClaimsService()._queue_claim_for_submission(
            sample_sha_claim, sample_sha_claim.created_by
        )
    ...
    def test_process_failed_claim_increments_retry(self, sha_client, sample_claim_with_items):
