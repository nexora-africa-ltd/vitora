...
class TestIPSBundleStructure:
    ...
    def test_ips_has_resource_type(self, valid_ips_bundle):
        ...
        """
        assert 'resourceType' in valid_ips_bundle, (
            "IPS must have 'resourceType' field"
        )
        assert valid_ips_bundle['resourceType'] == 'Bundle', (
    ...
    def test_ips_has_type_document(self, valid_ips_bundle):
        ...
        assert 'type' in valid_ips_bundle, (
            "IPS bundle must have 'type' field"
        )
        assert valid_ips_bundle['type'] == 'document', (
    ...
    def test_ips_has_id(self, valid_ips_bundle):
        ...
        """
        assert 'id' in valid_ips_bundle, (
            "IPS bundle must have 'id' field"
        )
        assert valid_ips_bundle['id'], (
            "IPS bundle id must not be empty"
        )
    ...
    def test_ips_has_timestamp(self, valid_ips_bundle):
        ...
        assert 'timestamp' in valid_ips_bundle, (
            "IPS bundle must have 'timestamp' field"
        )
        # Timestamp should be ISO 8601 format
        assert 'T' in valid_ips_bundle['timestamp'], (
            "timestamp must be ISO 8601 datetime format"
        )
    ...
    def test_ips_has_entry_array(self, valid_ips_bundle):
        ...
        """
        assert 'entry' in valid_ips_bundle, (
            "IPS bundle must have 'entry' field"
        )
        assert isinstance(valid_ips_bundle['entry'], list), (
            "entry must be an array"
        )
        assert len(valid_ips_bundle['entry']) > 0, (
            "entry array must have at least one resource"
        )
...
class TestIPSCompositionResource:
    ...
    def test_composition_has_status(self, valid_ips_bundle):
        ...
        assert 'status' in composition, (
            "Composition must have 'status' field"
        )
    ...
    def test_composition_has_subject(self, valid_ips_bundle):
        ...
        assert 'subject' in composition, (
            "Composition must have 'subject' field"
        )
        assert 'reference' in composition['subject'], (
            "Composition subject must have 'reference'"
        )
    ...
    def test_composition_has_date(self, valid_ips_bundle):
        ...
        assert 'date' in composition, (
            "Composition must have 'date' field"
        )
    ...
    def test_composition_has_author(self, valid_ips_bundle):
        ...
        assert 'author' in composition, (
            "Composition must have 'author' field"
        )
        assert len(composition['author']) > 0, (
            "author array must have at least one entry"
        )
    ...
    def test_composition_has_title(self, valid_ips_bundle):
        ...
        assert 'title' in composition, (
            "Composition must have 'title' field"
        )
...
class TestIPSCompositionSections:
    ...
    def test_composition_has_sections(self, valid_ips_bundle):
        ...
        assert 'section' in composition, (
            "Composition must have 'section' field"
        )
        assert len(composition['section']) > 0, (
            "Composition must have at least one section"
        )
    ...
    def test_allergies_section_present(self, valid_ips_bundle):
        ...
        assert allergy_section is not None, (
            "Composition should have an Allergies section"
        )
    ...
    def test_medication_section_present(self, valid_ips_bundle):
        ...
        assert med_section is not None, (
            "Composition should have a Medication List section"
        )
    ...
    def test_medication_section_has_entries(self, valid_ips_bundle):
        ...
        if med_section:
            ...
            assert isinstance(entries, list), (
                "Medication section entry must be an array"
            )
...
class TestIPSPatientResource:
    ...
    def test_ips_contains_patient_resource(self, valid_ips_bundle):
        ...
        assert patient, (
            "IPS bundle must contain a Patient resource"
        )
    ...
    def test_patient_has_identifier(self, valid_ips_bundle):
        ...
        assert 'identifier' in patient, (
            "Patient in IPS must have 'identifier' field"
        )
    ...
    def test_patient_has_name(self, valid_ips_bundle):
        ...
        assert 'name' in patient, (
            "Patient in IPS must have 'name' field"
        )
    ...
    def test_patient_has_gender(self, valid_ips_bundle):
        ...
        assert 'gender' in patient, (
            "Patient in IPS must have 'gender' field"
        )
    ...
    def test_entries_have_full_url(self, valid_ips_bundle):
        ...
        for entry in valid_ips_bundle.get('entry', []):
            assert 'fullUrl' in entry, (
                "Each bundle entry should have 'fullUrl'"
            )
    ...
    def test_entries_have_resource(self, valid_ips_bundle):
        ...
        for entry in valid_ips_bundle.get('entry', []):
            assert 'resource' in entry, (
                "Each bundle entry must have 'resource'"
            )
...
class TestIPSSummaryAPIEndpoint:
    ...
    def test_shr_summary_endpoint_configured(self, shr_api_endpoints):
        ...
        """
        assert 'shr_summary' in shr_api_endpoints, (
            "SHR API must include shr_summary endpoint"
        )
    ...
    def test_shr_summary_uses_get_method(self):
        ...
        assert expected_method == 'GET', (
            "IPS summary should use GET method"
        )
    ...
    def test_shr_summary_requires_cr_id(self):
        ...
        assert required_param, (
            "IPS summary endpoint must accept 'cr_id' query parameter"
        )
...
class TestIPSBundleWithDispenseHistory:
    ...
    def test_bundle_can_contain_medication_request(self, ips_bundle_with_dispense_history):
        ...
        assert has_med_request, (
            "IPS bundle with dispense history should contain MedicationRequest"
        )
