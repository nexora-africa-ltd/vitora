              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {item.completed_case_count} completed of {item.case_count} case
                    {item.case_count === 1 ? '' : 's'}
                  </p>
                </div>
...
export default function TheatreReportsPage() {
  ...
  const activeStatusCounts = useMemo(
    () =>
      (report?.status_breakdown ?? []).filter((item) =>
        ['PRE_OP', 'IN_THEATRE', 'IN_SURGERY', 'IN_PACU'].includes(item.status)
      ),
    [report]
  );
  ...
  const completionRate =
    totals && totals.case_count > 0
      ? Math.round((totals.completed_case_count / totals.case_count) * 100)
      : 0;
  const busiestDay = throughputSeries.reduce<(typeof throughputSeries)[number] | null>(
    (currentBest, item) => {
  ...
  const lowestUtilizationTheatre =
    utilizationByTheatre.length > 0 ? utilizationByTheatre[utilizationByTheatre.length - 1] : null;
  ...
  return (
    <div className="container mx-auto space-y-6 py-6">
          ...
          <div className="flex flex-wrap gap-2">
            {[7, 30, 90].map((days) => (
              <Button
                ...
                size="sm"
                onClick={() => beginRangeTransition(() => setRangeDays(days))}
                disabled={isPending && rangeDays === days}
              >
                {days}d
              </Button>
            ))}
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          ...
          { label: 'On-Time Starts %', value: onTimeStarts?.percent ?? 0, icon: TrendingUp },
          {
            ...
            value: report?.turnaround.average_minutes ?? 0,
            icon: Clock3,
          },
        ].map((card) => (
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">On-Time Starts</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {loading || isPending ? (
              ...
              <div className="py-10 text-center text-sm text-muted-foreground">
                No on-time start data available for this range.
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between rounded-lg border p-4">
                  ...
                  <Badge
                    variant={
                      onTimeStarts.percent >= 80
                        ? 'success'
                        : onTimeStarts.percent >= 60
                          ? 'warning'
                          : 'destructive'
                    }
                    size="sm"
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span>
                      {onTimeStarts.on_time_case_count} /{' '}
                      {Math.max(onTimeStarts.measured_case_count, 1)}
                    </span>
                  </div>
                  <div className="h-3 overflow-hidden rounded-full bg-muted">
                    <div className="flex h-full">
                      <div
                        ...
                        style={{
                          width: `${onTimeStarts.measured_case_count > 0 ? (onTimeStarts.on_time_case_count / onTimeStarts.measured_case_count) * 100 : 0}%`,
                        }}
                      />
                      <div
                        className="bg-amber-400"
                        style={{
                          width: `${onTimeStarts.measured_case_count > 0 ? (onTimeStarts.late_case_count / onTimeStarts.measured_case_count) * 100 : 0}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Throughput by Day</CardTitle>
          </CardHeader>
          <CardContent className="h-80">
            {loading || isPending ? (
              ...
              <div className="py-10 text-center text-sm text-muted-foreground">
                No throughput data available for this range.
              </div>
            ) : (
        ...
        <Card>
          ...
          <CardContent className="h-80">
            {loading || isPending ? (
              ...
              <div className="py-10 text-center text-sm text-muted-foreground">
                No case status mix available for this range.
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Theatre Performance Detail</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {utilizationByTheatre.length === 0 ? (
              <div className="py-8 text-sm text-muted-foreground">
                No theatre-level performance rows are available for this range.
              </div>
            ) : (
              utilizationByTheatre.map((item) => (
                <div key={item.theatre_id} className="rounded-lg border p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.theatre_code} • {item.completed_case_count}/{item.case_count}{' '}
                        completed
                      </p>
                    </div>
                    <Badge
                      variant={
                        item.utilization_percent >= 80
                          ? 'success'
                          : item.utilization_percent >= 60
                            ? 'info'
                            : 'outline'
                      }
                      size="sm"
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-3">
                    ...
                    <div className="rounded-md bg-muted/50 px-3 py-2">
                      ...
                      <p className="text-sm font-medium">
                        {item.turnaround_average_minutes || 0} min
                      </p>
                    </div>
                    <div className="rounded-md bg-muted/50 px-3 py-2">
                      <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Bottleneck Watch</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="rounded-lg border p-3">
              <p className="text-sm font-medium">Late starts</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {onTimeStarts?.late_case_count ?? 0} of {onTimeStarts?.measured_case_count ?? 0}{' '}
                measured starts exceeded the {onTimeStarts?.threshold_minutes ?? 15} minute
                tolerance.
              </p>
              <Badge
                variant={
                  lateStartsPercent <= 20
                    ? 'success'
                    : lateStartsPercent <= 40
                      ? 'warning'
                      : 'destructive'
                }
                size="sm"
            ...
            <div className="rounded-lg border p-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {busiestDay
                  ? `${busiestDay.date} carried ${busiestDay.case_count} scheduled cases and ${Math.round((busiestDay.scheduled_minutes / 60) * 10) / 10} booked hours.`
                  : 'No daily throughput data is available.'}
              </p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-sm font-medium">Utilization spread</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {highestUtilizationTheatre
                  ? `${highestUtilizationTheatre.theatre_name} leads at ${highestUtilizationTheatre.utilization_percent}% utilization.`
                  : 'No theatre utilization data available.'}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {lowestUtilizationTheatre
                  ? `${lowestUtilizationTheatre.theatre_name} is lowest at ${lowestUtilizationTheatre.utilization_percent}% utilization.`
                  : ''}
              </p>
            </div>
            <div className="rounded-lg border p-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {surgeonWorkload[0]
                  ? `${surgeonWorkload[0].clinician_name} carries the highest surgical load with ${surgeonWorkload[0].case_count} cases over ${formatHours(surgeonWorkload[0].scheduled_minutes)} booked.`
                  : 'No surgeon workload data available.'}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Utilization by Theatre</CardTitle>
          </CardHeader>
          <CardContent className="h-80">
            {loading || isPending ? (
              <div className="py-10 text-center text-sm text-muted-foreground">
              ...
              <div className="py-10 text-center text-sm text-muted-foreground">
                No theatre utilization data available for this range.
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={utilizationByTheatre}
                  layout="vertical"
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <span className="text-sm font-medium">Active workflow cases</span>
              <Badge variant="outline" size="sm" className="w-fit">
                {totals?.active_case_count ?? 0}
              </Badge>
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <span className="text-sm font-medium">Report window</span>
              <Badge variant="outline" size="sm" className="w-fit">
                Last {rangeDays} days
              </Badge>
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <span className="text-sm font-medium">Average daily throughput</span>
              <Badge variant="info" size="sm" className="w-fit">
                {totals?.average_daily_throughput ?? 0}
              </Badge>
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <span className="text-sm font-medium">Cases with turnaround measured</span>
              <Badge variant="info" size="sm" className="w-fit">
                {report?.turnaround.cases_with_measurement_count ?? 0}
              </Badge>
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <span className="text-sm font-medium">Booked vs available hours</span>
              <Badge variant="outline" size="sm" className="w-fit">
                {Math.round(((totals?.scheduled_minutes ?? 0) / 60) * 10) / 10} /{' '}
                {Math.round(((totals?.available_minutes ?? 0) / 60) * 10) / 10}h
              </Badge>
            </div>
            ...
            {utilizationByTheatre[0] ? (
              <div className="rounded-lg border border-dashed p-3 text-sm text-muted-foreground">
                Highest booked theatre:{' '}
                <span className="font-medium text-foreground">
                  {utilizationByTheatre[0].theatre_name}
                </span>{' '}
                at{' '}
                <span className="font-medium text-foreground">
                  {utilizationByTheatre[0].utilization_percent}%
                </span>{' '}
                booked utilization.
              </div>
            ) : null}
