...
import { Loader2, Plus, AlertTriangle, Package, ArrowDownToLine } from 'lucide-react';
...
import type { VaccineStockListItem, StockIssueData } from '@/lib/types/immunizations';
        ...
        <ResponsiveTable
          ...
          columns={[
            ...
            },
            {
              ...
              cell: (s) => (
                <span
                  className={`text-sm ${s.is_expired ? 'font-medium text-destructive' : s.is_near_expiry ? 'text-orange-600' : ''}`}
                >
            ...
            },
            {
              ...
              cell: (s) => (
                <div className="flex flex-wrap gap-1">
                  ...
                  {s.is_near_expiry && !s.is_expired && (
                    <Badge className="bg-orange-100 text-xs text-orange-800">Near Expiry</Badge>
                  )}
                  {s.is_low_stock && (
                    <Badge className="bg-yellow-100 text-xs text-yellow-800">Low</Badge>
                  )}
                  {!s.is_expired && !s.is_near_expiry && !s.is_low_stock && (
                    <Badge className="bg-green-100 text-xs text-green-800">OK</Badge>
                  )}
                </div>
              ),
            },
            ...
            <Card className="p-3">
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  <p className="truncate font-medium">{s.vaccine_name}</p>
                  <p className="text-xs text-muted-foreground">{s.batch_number}</p>
            ...
            <div className="space-y-3 pt-2 sm:space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    setVaccineId(v);
                    setReceiveTouched((t) => ({ ...t, vaccine: true }));
                  }}
                >
                  <SelectTrigger
                    className={
                      receiveTouched.vaccine && !vaccineId
                ...
                {receiveTouched.vaccine && !vaccineId && (
                  <p className="mt-1 text-xs text-destructive">Vaccine is required</p>
                )}
              </div>
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3">
                <div>
                  ...
                  <Input
                    ...
                    className={
                      receiveTouched.batchNumber && !batchNumber
                        ? 'border-destructive focus-visible:ring-destructive'
                        : ''
                    }
                  />
                  {receiveTouched.batchNumber && !batchNumber && (
                    <p className="mt-1 text-xs text-destructive">Batch number is required</p>
                  )}
                </div>
                <div>
                  <Label>
                  ...
                  <Input
                    ...
                    placeholder="e.g. 500"
                    className={
                      receiveTouched.quantity && !quantity
                        ? 'border-destructive focus-visible:ring-destructive'
                        : ''
                    }
                  />
                  {receiveTouched.quantity && !quantity && (
                    <p className="mt-1 text-xs text-destructive">Quantity is required</p>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3">
                <div>
                  ...
                  <Input
                    ...
                    className={
                      receiveTouched.expiryDate && !expiryDate
                        ? 'border-destructive focus-visible:ring-destructive'
                        : ''
                    }
                  />
                  {receiveTouched.expiryDate && !expiryDate && (
                    <p className="mt-1 text-xs text-destructive">Expiry date is required</p>
                  )}
                </div>
                <div>
              ...
              <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                ...
                >
                  {receiveMutation.isPending && <Loader2 className="mr-1 h-4 w-4 animate-spin" />}
                  Receive
                </Button>
              </div>
            </div>
          ...
          <DialogContent>
            ...
            <div className="space-y-3 pt-2 sm:space-y-4">
              <div>
                ...
                <Select
                  value={issueType}
                  onValueChange={(v) => setIssueType(v as IssueTransactionType)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                ...
                >
                  {issueMutation.isPending && <Loader2 className="mr-1 h-4 w-4 animate-spin" />}
                  Submit
                </Button>
              </div>
            </div>
