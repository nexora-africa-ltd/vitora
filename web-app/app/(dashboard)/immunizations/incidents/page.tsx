        ...
        <ResponsiveTable
          ...
          columns={[
            ...
            },
            {
              ...
              sortable: true,
              cell: (i) => (
                <Badge className={`${severityColors[i.severity]} w-fit shrink-0`}>
                  {i.severity}
                </Badge>
              ),
            },
            {
              ...
              sortable: true,
              cell: (i) => (
                <Badge className={`${statusColors[i.status]} w-fit shrink-0`}>{i.status}</Badge>
              ),
            },
            {
              key: 'doses_lost',
              ...
              cell: (i) => (
                <span
                  className={`text-sm ${i.doses_lost > 0 ? 'font-medium text-destructive' : ''}`}
                >
            ...
            <Card className="p-3">
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  ...
                  {i.doses_lost > 0 && (
                    <p className="mt-1 text-xs text-destructive">{i.doses_lost} doses lost</p>
                  )}
                </div>
                <div className="flex flex-col items-end gap-1">
                  <Badge className={`${severityColors[i.severity]} w-fit shrink-0 text-xs`}>
                    {i.severity}
                  </Badge>
                  <Badge className={`${statusColors[i.status]} w-fit shrink-0 text-xs`}>
                    {i.status}
                  </Badge>
                  {(i.status === 'OPEN' || i.status === 'INVESTIGATING') && (
                    <Button size="sm" variant="ghost" onClick={() => openResolveDialog(i)}>
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3">
                <div>
                  ...
                  <Input
                    type="number"
                    value={dosesAffected}
                    onChange={(e) => setDosesAffected(e.target.value)}
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    value={dosesLost}
                    onChange={(e) => setDosesLost(e.target.value)}
                    placeholder="Confirmed lost"
                  />
                </div>
              </div>
        ...
        <Dialog open={resolveDialogOpen} onOpenChange={setResolveDialogOpen}>
          <DialogContent>
            ...
            <div className="space-y-3 pt-2 sm:space-y-4">
              ...
              <div>
                <Label>
                  Corrective Actions <span className="text-destructive">*</span>
                </Label>
                <Textarea
                  ...
                  rows={3}
                  className={
                    resolveTouched.correctiveActions && !correctiveActions
                      ? 'border-destructive focus-visible:ring-destructive'
                      : ''
                  }
                />
                {resolveTouched.correctiveActions && !correctiveActions && (
                  <p className="mt-1 text-xs text-destructive">Corrective actions are required</p>
                )}
              </div>
              <div>
              ...
              <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                ...
                >
                  {resolveMutation.isPending && <Loader2 className="mr-1 h-4 w-4 animate-spin" />}
                  Resolve
                </Button>
              </div>
            </div>
