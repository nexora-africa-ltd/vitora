...
import type { WalkInCustomer, WalkInCustomerCreateData } from '@/lib/types/standalone-pharmacy';
...
export default function WalkInCustomersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['walkin-customers', search],
    queryFn: () => standalonePharmacyApi.listWalkInCustomers({ search: search || undefined }),
  });

  const createMutation = useMutation({
    mutationFn: (data: WalkInCustomerCreateData) =>
      standalonePharmacyApi.createWalkInCustomer(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['walkin-customers'] });
      setShowCreate(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Name',
      sortable: true,
      sortFn: (a: WalkInCustomer, b: WalkInCustomer) =>
    ...
    },
    {
      key: 'national_id',
      header: 'ID',
      cell: (item: WalkInCustomer) => item.national_id || '-',
      hideOnMobile: true,
    },
    {
      key: 'phone_number',
      header: 'Phone',
      cell: (item: WalkInCustomer) => item.phone_number || '-',
      hideOnMobile: true,
    },
    {
      key: 'linked',
    ...
    },
    {
      ...
      cell: (item: WalkInCustomer) => new Date(item.created_at).toLocaleDateString(),
      hideOnMobile: true,
    },
  ];
    ...
    <PullToRefresh onRefresh={refresh} isRefreshing={isRefreshing}>
      <div className="space-y-4 sm:space-y-6">
        <PageHeader
          ...
          helpContent="Register and manage walk-in pharmacy customers (OTC sales, external prescriptions). Walk-in customers don't require a full HMIS registration."
          actions={
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveTable
              ...
              isLoading={isLoading}
              emptyMessage="No walk-in customers registered yet."
            />
          </CardContent>
        </Card>
      </div>
...
function WalkInCreateForm({
  ...
  onSubmit: (data: WalkInCustomerCreateData) => void;
  isLoading: boolean;
}) {
  const [formData, setFormData] = useState<WalkInCustomerCreateData>({
    first_name: '',
    last_name: '',
    gender: '',
    phone_number: '',
