...
export default function AnnualReportDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 sm:space-y-6">
        <PageHeader title="Annual Report" />
  ...
  if (error || !report) {
    return (
      <div className="space-y-4 sm:space-y-6">
        <PageHeader title="Annual Report" />
        <Card className="p-6">
          <p className="text-destructive">Failed to load annual report.</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <PageHeader
        title={`${report.clinic_name} — ${report.year}`}
        helpContent="Annual report aggregated from quarterly (or monthly) clinic reports. Shows yearly totals for visits, demographics, chronic care, ANC, and revenue."
      />
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center p-3 sm:p-4 rounded-lg bg-muted/50">
        <div className="flex flex-col gap-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {report.year} • {report.quarterly_report_ids.length} quarterly reports aggregated
          </p>
        </div>
        <Badge
