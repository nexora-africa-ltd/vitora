/**
 * Physiotherapy Sessions List Page
 * View all physiotherapy sessions with filtering and status management
 */
...
import {
  ...
  useMarkPhysioSessionNoShow,
} from '@/lib/hooks/use-physiotherapy';
import { useToast } from '@/lib/hooks/use-toast';
import type { PhysiotherapySessionListParams } from '@/lib/types/physiotherapy';
...
export default function PhysiotherapySessionsPage() {
  ...
  // Build query params
  const params: PhysiotherapySessionListParams = {
    page,
    page_size: 20,
  ...
  const { data, isLoading, error } = usePhysioSessions(params);
  const startMutation = useStartPhysioSession();
  const cancelMutation = useCancelPhysioSession();
  const noShowMutation = useMarkPhysioSessionNoShow();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        helpContent="View and manage all physiotherapy treatment sessions. Filter by date, status, or search for specific patients."
      />
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="No physiotherapy sessions match your filters. Try adjusting the date range or status filter."
            />
          ) : (
            <>
              <Table>
