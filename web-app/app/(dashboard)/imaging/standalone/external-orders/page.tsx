...
export default function ExternalImagingOrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['external-imaging-orders'],
    queryFn: () => standaloneImagingApi.listExternalOrders(),
  });
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }: { id: number; reason: string }) =>
      standaloneImagingApi.rejectExternalOrder(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['external-imaging-orders'] });
  ...
  const columns = [
    {
      ...
      cell: (item: ExternalImagingOrderRequest) => (
        <span className="font-mono text-sm">
          {item.placer_order_number || item.message_control_id}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: 'Patient',
      sortable: true,
      cell: (item: ExternalImagingOrderRequest) => item.patient_name,
    },
    {
      key: 'procedures',
      header: 'Procedures',
      cell: (item: ExternalImagingOrderRequest) => (
        <span className="text-sm text-muted-foreground">
          {item.requested_procedures.length} procedure(s)
        </span>
      ),
    },
    {
  ...
  return (
    <PullToRefresh onRefresh={refresh} isRefreshing={isRefreshing}>
      <div className="space-y-4 sm:space-y-6">
        <PageHeader
          title="External Imaging Orders"
          helpContent="Inbound imaging orders from external systems via HL7. Accept to create an imaging order, or reject with a reason."
        />
