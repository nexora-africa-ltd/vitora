/**
 * New Facility Page (scoped to an organization)
 * Creates a facility pre-linked to the parent organization.
 */
'use client';
...
export default function NewFacilityPage() {
  ...
  const createFacility = useMutation({
    ...
    onError: (err: unknown) => {
      // Surface field-level validation errors from the API (e.g. duplicate MFL code)
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <PageHeader
        title="New Facility"
        helpContent={`Add a new facility to ${org?.name ?? 'this organization'}. Module capabilities will default based on the KEPH level selected.`}
      />
