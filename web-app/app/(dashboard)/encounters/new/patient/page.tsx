  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card>
        <CardHeader className="px-3 py-3 sm:px-6 sm:py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base sm:text-lg">Encounter Type</CardTitle>
            </div>
            <div className="flex items-center gap-2">
              <Select value={encounterType} onValueChange={handleEncounterTypeChange}>
        ...
        {encounterType === 'IPD' && (
          <CardContent className="space-y-3 px-3 pb-3 pt-0 sm:px-6 sm:pb-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
              <Label className="shrink-0 text-sm font-medium">Admission Urgency</Label>
              <Select value={admissionUrgency || 'ROUTINE'} onValueChange={handleUrgencyChange}>
        ...
        {encounterType === 'EMERGENCY' && (
          <CardContent className="px-3 pb-3 pt-0 sm:px-6 sm:pb-4">
            <Alert className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/20">
              <AlertTriangle className="h-4 w-4 text-amber-600" />
              <AlertDescription className="text-amber-700 dark:text-amber-300">
        ...
        <CardContent className="px-3 sm:px-6">
          ...
          {patientId && selectedPatient && (
            <div className="mt-3 space-y-2">
              ...
              {shaNumber && <SHABenefitsAlert shaEligible benefitsState={benefitsState} />}
            </div>
          )}
        </CardContent>
      </Card>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <Label
                  ...
                  className="flex cursor-pointer items-center gap-2 text-base font-semibold sm:text-lg"
                >
                  <Activity className="h-4 w-4 sm:h-5 sm:w-5" />
                  Record Vitals Now?
                </Label>
                <CardDescription className="mt-1">
                  Optionally record vital signs during registration. If skipped, vitals must be
                  recorded during triage before consultation.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
      ...
      {patientId && recordVitalsNow && (
        <VitalsForm data={formData} onChange={handleVitalChange} patient={selectedPatient} />
      )}
      ...
      {!canProceed && (
        <Alert variant="destructive" className="bg-destructive/10">
          ...
          <AlertDescription>Please select a patient to continue.</AlertDescription>
        </Alert>
      )}
