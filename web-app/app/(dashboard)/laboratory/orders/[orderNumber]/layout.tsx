 ...
 */
export default function LabOrderDetailLayout({ children }: { children: React.ReactNode }) {
  const params = useParams();
  const orderNumber = params.orderNumber as string;
  const { data: order } = useLabOrder(orderNumber);

  return <PatientProvider patientId={order?.patient ?? null}>{children}</PatientProvider>;
}
