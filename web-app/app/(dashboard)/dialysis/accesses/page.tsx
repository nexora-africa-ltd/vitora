  ...
  const columns = [
    {
      ...
      cell: (item: VascularAccess) => <span className="font-medium">Patient #{item.patient}</span>,
    },
    {
      key: 'access_type',
      header: 'Type',
    ...
    },
    {
      ...
      cell: (item: VascularAccess) => (
        <Badge className={`${ACCESS_STATUS_COLORS[item.status]} w-fit shrink-0`}>
          {item.status}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      hideOnMobile: true,
      cell: (item: VascularAccess) =>
        item.last_assessment_date ? formatDate(item.last_assessment_date) : '—',
    },
  ];
        ...
        <PageHeader
          ...
          actions={
            <PermissionGate action="dialysis.manage">
              <Button
                onClick={() => router.push('/dialysis/accesses/new')}
                disabled={!canCreateRoute('/dialysis/accesses/new')}
              >
