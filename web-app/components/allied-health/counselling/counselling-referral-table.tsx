/**
 * Counselling Referral Table
 * List view for counselling referrals
 */
...
import { useCounsellingReferrals } from '@/lib/hooks/use-counselling';
import type {
...
interface CounsellingReferralTableProps {
  initialParams?: CounsellingReferralListParams;
}

export function CounsellingReferralTable({ initialParams }: CounsellingReferralTableProps) {
  const router = useRouter();
  const [params, setParams] = useState<CounsellingReferralListParams>({
    page: 1,
    page_size: 20,
    ...initialParams,
  });
  ...
  const { data, isLoading, error } = useCounsellingReferrals(params);
  ...
  const handleRowClick = (referral: CounsellingReferralListItem) => {
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="No counselling referrals match your criteria"
          action={{
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Showing {data.results.length} of {data.count} referrals
            </div>
            <div className="flex gap-2">
              <Button
                variant="outline"
