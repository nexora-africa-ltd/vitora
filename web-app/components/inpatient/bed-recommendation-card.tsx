    ...
    <div className="space-y-2">
      ...
      <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
        {predictions.slice(0, 4).map((prediction) => (
          <div
            key={prediction.admission_id}
            className="flex flex-col gap-1 border-b pb-2 last:border-b-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between"
          >
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {prediction.source === 'expected_discharge'
                  ? 'Clinician-set discharge date'
                  : 'Estimated from average LOS'}
              </p>
            </div>
...
const SCORE_HELP: Record<
  string,
  { help: string; icon: typeof TrendingUp; iconColor: string; bg: string }
> = {
  workload: {
...
export function BedRecommendationCard({
  ...
  const predictedDischarges =
    'predicted_discharges' in recommendation ? recommendation.predicted_discharges : [];
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-center gap-2">
            <CardTitle className="text-lg">
              {strategy === 'SMART' ? 'Smart bed recommendation' : 'Rules-based recommendation'}
            </CardTitle>
            <HelpPopover
              content={
                strategy === 'SMART'
      ...
      <CardContent className="space-y-4">
        {isSuccess ? (
          <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-semibold tracking-tight">
                      {recommendation.assigned_bed_number}
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  {'evaluation_time_ms' in recommendation && (
                    <Badge variant="secondary" className="text-xs">
                      {recommendation.evaluation_time_ms} ms
                    </Badge>
                  )}
            ...
            {'emergency_buffer_enforced' in recommendation &&
              recommendation.emergency_buffer_enforced && (
                <Alert>
                  <ShieldAlert className="h-4 w-4" />
                  <AlertDescription>
                    Emergency buffer rules were enforced. Beds reserved for emergency admissions
                    were excluded from this recommendation.
                  </AlertDescription>
                </Alert>
              )}

            {'infection_isolation_triggered' in recommendation &&
              recommendation.infection_isolation_triggered && (
                <Alert>
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    Infection control signals influenced this recommendation. Isolation capability
                    was prioritized during scoring.
                  </AlertDescription>
                </Alert>
              )}

            {'smart_scores' in recommendation &&
              Object.keys(recommendation.smart_scores).length > 0 && (
                      ...
                      return (
                        <div key={key} className={`rounded-xl border p-3 ${scoreInfo.bg}`}>
                          ...
                          {scoreInfo.numericValue !== null && (
                            <Progress
                              value={Math.min(scoreInfo.numericValue * 100, 100)}
            ...
            {'candidates_evaluated' in recommendation &&
              recommendation.candidates_evaluated.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-2">
                    {recommendation.candidates_evaluated.slice(0, 3).map((candidate, index) => (
                      <div
                        key={candidate.bed_id}
                        className={`flex flex-col gap-2 rounded-xl border p-3 sm:flex-row sm:items-center sm:justify-between ${index === 0 ? 'border-primary/20 bg-primary/5' : 'bg-muted/20'}`}
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold ${index === 0 ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}
                          >
                            {index + 1}
                          </div>
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {candidate.passed
                                ? candidate.matched_constraints.join(', ') ||
                                  'Passed all constraints'
                                : candidate.rejection_reason ||
                                  candidate.failed_constraints.join(', ')}
                            </p>
                          </div>
                        </div>
                        <Badge
            ...
            {'candidates_evaluated' in recommendation &&
              recommendation.candidates_evaluated.length > 0 && (
                <div className="rounded-lg border bg-muted/20 p-3">
                  <p className="text-sm font-medium">
                    Candidates were evaluated but none passed all constraints.
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Review the ward rules or switch to manual selection if you need to proceed with
                    a justified override.
                  </p>
                </div>
              )}
          </div>
