...
const IMAGING_STATUS_CONFIG: Record<
  ImagingOrderStatus,
  { label: string; color: string; icon: React.ElementType }
> = {
  DRAFT: {
...
export function AdmissionOrdersTab({
  admissionId,
  patientId,
  encounterId,
  isActive = true,
}: AdmissionOrdersTabProps) {
  ...
  const procOrders = (procOrdersData?.results ??
    []) as import('@/lib/types/procedure').ProcedureOrderListItem[];

  // Sheet state for each order type
  const [activeSheet, setActiveSheet] = useState<'lab' | 'imaging' | 'rx' | 'procedure' | null>(
    null
  );
  ...
  const totalOrders =
    labOrders.length + imagingOrders.length + prescriptions.length + procOrders.length;
      ...
      {isActive && (
        <div className="grid grid-cols-4 gap-2 sm:flex sm:flex-wrap">
          ...
          <Button
            ...
            size="sm"
            className="w-full sm:w-auto sm:px-3"
            onClick={() => setActiveSheet('imaging')}
            title="New Imaging"
          >
          ...
          <Button
            ...
            size="sm"
            className="w-full sm:w-auto sm:px-3"
            onClick={() => setActiveSheet('procedure')}
            title="New Procedure"
          >
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <OrdersSummaryCard
            ...
            items={procOrders.map((o) => ({
              ...
              statusConfig: {
                label:
                  PROCEDURE_STATUS_LABELS[o.status as keyof typeof PROCEDURE_STATUS_LABELS] ??
                  o.status,
                color:
                  PROCEDURE_STATUS_COLORS[o.status as keyof typeof PROCEDURE_STATUS_COLORS] ??
              ...
              ...(o.priority !== 'ROUTINE'
                ? {
                    priorityConfig: {
                      ...
                      color:
                        PROCEDURE_PRIORITY_COLORS[
                          o.priority as keyof typeof PROCEDURE_PRIORITY_COLORS
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1.5">
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <Badge
                  className={`${item.statusConfig.color} px-1.5 py-0 text-[10px]`}
                  variant="secondary"
                >
                ...
                {item.priorityConfig && item.priorityConfig.label !== 'Routine' && (
                  <Badge
                    className={`${item.priorityConfig.color} px-1 py-0 text-[10px]`}
                    variant="secondary"
                  >
        ...
        {items.length > 3 && (
          <p className="pt-1 text-xs text-muted-foreground">+{items.length - 3} more</p>
        )}
      </CardContent>
    </Card>
  );
