...
const pacuDocumentationSchema = z
  .object({
  ...
  .superRefine((values, ctx) => {
    if (
      (values.nausea_vomiting ||
        values.shivering ||
        values.respiratory_issues ||
        values.cardiovascular_issues) &&
...
export function PostOpWorkspace({
  surgeryCase,
  onCaseRefresh,
}: {
  ...
  const loadRecord = useCallback(
    async (showLoading = true) => {
      ...
      try {
        const [record, postOpPlans] = await Promise.all([
          ...
          aiApi
            .getStoredSurgicalPostOpCarePlans({ surgery_case_id: surgeryCase.id })
            .catch(() => []),
        ]);
        ...
        if (record) {
          ...
          dischargeForm.reset({
            ...
            discharge_destination: (record.discharge_destination || 'WARD') as
              | 'WARD'
              | 'ICU'
              | 'DAY_CASE_DISCHARGE'
    ...
    [
      ...
      documentationForm,
      surgeryCase.id,
      surgeryCase.case_number,
      surgeryCase.requesting_doctor,
    ]
  );
  ...
  const mappedProcedureKey =
    surgeryCase.primary_procedure_tibabot_key ||
    surgeryCase.ai_surgical_summary.post_op.procedure_key ||
    '';
  const latestStoredPostOp = storedPostOpPlans[0] ?? null;
  const latestStoredPostOpData = latestStoredPostOp?.result_data as
    | Record<string, unknown>
    | undefined;
  const latestSurgicalApgar = latestStoredPostOpData?.surgical_apgar as
    | Record<string, unknown>
    | undefined;
  ...
  const {
    getLink,
    actionLink,
    hasOrders: postOpHasOrders,
  } = useAdvisoryLinks(postOpResultId, 'post_op_care_plan');
  ...
  const createArrival = async (values: z.infer<typeof pacuArrivalSchema>) => {
    try {
      ...
      toast({
        title: 'PACU arrival recorded',
        description: 'Recovery monitoring can now continue in this workspace.',
      });
      await refreshAll();
    } catch (error) {
      toast({
        title: 'Unable to create PACU record',
  ...
  const addVital = async (values: z.infer<typeof pacuVitalSchema>) => {
    try {
      ...
      toast({
        title: 'PACU vital saved',
        description: 'The recovery monitoring trend has been updated.',
      });
      vitalForm.reset({ ...values, notes: '' });
      await refreshAll();
    } catch (error) {
      toast({
        title: 'Unable to save PACU vital',
        description: getApiErrorMessage(error),
        variant: 'destructive',
      });
    }
  ...
  const saveDocumentation = async (values: z.infer<typeof pacuDocumentationSchema>) => {
    try {
      ...
      toast({
        title: 'PACU documentation saved',
        description: 'Recovery issues, medications, and handover notes were updated.',
      });
      await refreshAll();
    } catch (error) {
      toast({
        title: 'Unable to save PACU documentation',
        description: getApiErrorMessage(error),
        variant: 'destructive',
      });
    }
  ...
  const downloadSummary = async () => {
    try {
      ...
      toast({
        title: 'Unable to download PACU summary',
        description: getApiErrorMessage(error),
        variant: 'destructive',
      });
    }
  ...
  const dischargePatient = async (values: z.infer<typeof pacuDischargeSchema>) => {
    try {
      ...
      if (!documentationResult.success) {
        ...
        if (issue?.path[0]) {
          documentationForm.setError(
            issue.path[0] as keyof z.infer<typeof pacuDocumentationSchema>,
            {
        ...
        toast({
          title: 'PACU documentation incomplete',
          description: 'Resolve documentation issues before discharge.',
          variant: 'destructive',
        });
        return;
      }
      ...
      toast({
        title: 'PACU discharge completed',
        description: 'Recovery discharge details were recorded.',
      });
      await refreshAll();
    } catch (error) {
      toast({
        title: 'Unable to complete PACU discharge',
        description: getApiErrorMessage(error),
        variant: 'destructive',
      });
    }
    ...
    try {
      ...
      await aiApi.generateSurgicalPostOpCarePlan({
        ...
        estimated_blood_loss_ml:
          estimatedBloodLossMl === '' ? undefined : Number(estimatedBloodLossMl),
      ...
      toast({
        title: 'Surgical AI advisory complete',
        description: 'Post-op care plan and recovery guidance are now available below.',
      });
      await refreshAll();
      ...
      toast({
        title: 'Unable to generate care plan',
        description: getApiErrorMessage(error),
        variant: 'destructive',
      });
    } finally {
      ...
      >
        ...
        <CardHeader className="relative pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            <Badge
              variant={surgeryCase.ai_surgical_summary.post_op.has_result ? 'success' : 'outline'}
              size="sm"
        ...
        <CardContent className="relative space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(event) =>
                  setEstimatedBloodLossMl(event.target.value ? Number(event.target.value) : '')
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(event) =>
                  setLowestHeartRate(event.target.value ? Number(event.target.value) : '')
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={lowestMap}
                onChange={(event) =>
                  setLowestMap(event.target.value ? Number(event.target.value) : '')
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={capriniScore}
                onChange={(event) =>
                  setCapriniScore(event.target.value ? Number(event.target.value) : '')
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              rows={3}
              value={postOpFindings}
              onChange={(event) => setPostOpFindings(event.target.value)}
              placeholder="Key findings, drains, stoma, or intra-op concerns"
            />
          </div>

          <Button
            type="button"
            onClick={() => void generatePostOpCarePlan()}
            disabled={generatingCarePlan || !mappedProcedureKey || postOpHasOrders}
          >
            {generatingCarePlan ? (
          ...
          {latestStoredPostOp ? (
            <div className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-lg border p-3 text-sm">
                  ...
                  <p className="mt-1 font-medium">
                    {latestStoredPostOp.surgical_apgar_score ??
                      latestSurgicalApgarScore ??
                      'Unavailable'}
                  </p>
                </div>
                <div className="rounded-lg border p-3 text-sm">
                  ...
                  <p className="mt-1 font-medium">
                    {latestStoredPostOp.risk_level || latestSurgicalApgarRiskLevel || 'Unavailable'}
                  </p>
                </div>
                <div className="rounded-lg border p-3 text-sm xl:col-span-2">
                  ...
                  <p className="mt-1 font-medium">
                    {String(
                      latestStoredPostOpData?.monitoring || 'No monitoring guidance recorded'
                    )}
                  </p>
                </div>
              </div>
              ...
              {(() => {
                ...
                const followUp = latestStoredPostOpData?.follow_up as
                  | Record<string, unknown>
                  | undefined;
                return (
                  <>
                    <div className="flex justify-end">
                      <Button
                        ...
                        onClick={() =>
                          setPostOpOpenSections(allExpanded ? [] : ALL_POST_OP_SECTIONS)
                        }
                      >
                        {allExpanded ? (
                    ...
                    <Accordion
                      type="multiple"
                      className="w-full"
                      value={postOpOpenSections}
                      onValueChange={setPostOpOpenSections}
                    >
                      {/* Medications */}
                      <AccordionItem value="medications">
                        <AccordionTrigger className="text-sm font-medium">
                          <span className="flex items-center gap-2">
                            Medications
                            {Array.isArray(latestStoredPostOpData?.medications) &&
                            (latestStoredPostOpData.medications as string[]).length > 0 ? (
                              <Badge
                                variant="secondary"
                      ...
                      {Array.isArray(latestStoredPostOpData?.complications_to_watch) &&
                      (latestStoredPostOpData.complications_to_watch as Record<string, unknown>[])
                        .length > 0 ? (
                        <AccordionItem value="complications">
                          <AccordionTrigger className="text-sm font-medium">
                            <span className="flex items-center gap-2">
                              ...
                              >
                                {
                                  (latestStoredPostOpData.complications_to_watch as unknown[])
                                    .length
                                }
                              </Badge>
                            </span>
                          ...
                          <AccordionContent>
                            <div className="space-y-3 text-sm">
                              {(
                                latestStoredPostOpData.complications_to_watch as Record<
                                  string,
                                  unknown
                                >[]
                              ).map((comp, i) => (
                      ...
                      {/* Discharge Criteria */}
                      {Array.isArray(latestStoredPostOpData?.discharge_criteria) &&
                      (latestStoredPostOpData.discharge_criteria as string[]).length > 0 ? (
                        <AccordionItem value="discharge">
                          ...
                          <AccordionContent>
                            <ul className="ml-4 list-disc space-y-1 text-sm">
                              {(latestStoredPostOpData.discharge_criteria as string[]).map(
                                (item, i) => (
                                  <li key={i} className="flex items-center gap-1">
                        ...
                        <AccordionItem value="follow-up">
                          <AccordionTrigger className="text-sm font-medium">
                            <span className="flex items-center gap-2">
                              ...
                              <Badge
                                ...
                                className="h-5 min-w-5 justify-center rounded-full bg-emerald-500/15 px-1.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400"
                              >
                                {(followUp.appointment ? 1 : 0) +
                                  (followUp.investigations ? 1 : 0) +
                                  (Array.isArray(followUp.red_flags)
                                    ? (followUp.red_flags as unknown[]).length
                                    : 0)}
                              </Badge>
                            </span>
                          </AccordionTrigger>
                          <AccordionContent>
                            <div className="space-y-3 text-sm">
                              ...
                              {Array.isArray(followUp.red_flags) &&
                              (followUp.red_flags as string[]).length > 0 ? (
                                <div>
            ...
            <p className="text-sm text-muted-foreground">
              No advisory post-operative care plan has been generated for this case yet.
            </p>
          )}
        </CardContent>
      </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-base">
              ...
              PACU Arrival
              <HelpPopover content="Document patient arrival in the Post-Anesthesia Care Unit. Records arrival time, receiving nurse, initial Aldrete score, and handover details from the anesthesia team." />
            </CardTitle>
          </CardHeader>
          <CardContent>
            {surgeryCase.status !== 'IN_PACU' ? (
              <Alert>
                ...
                <AlertDescription>
                  Transition the case to PACU from the case workflow before creating the recovery
                  record.
                </AlertDescription>
              </Alert>
            ) : null}
              ...
              <form className="space-y-4" onSubmit={arrivalForm.handleSubmit(createArrival)}>
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Receiving nurse user ID</FormLabel>
                        <FormControl>
                  ...
                  <FormField
                    control={arrivalForm.control}
                    name="initial_aldrete_score"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Initial Aldrete score</FormLabel>
                        <FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Initial pain score</FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            min={0}
        ...
        <>
            ...
            <Card className="relative overflow-hidden">
              ...
              <CardHeader className="relative pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  Recovery Status
                  <HelpPopover content="At-a-glance recovery metrics: Aldrete scores, vital count, complication flags, and discharge readiness." />
                </CardTitle>
              </CardHeader>
              <CardContent className="relative space-y-3">
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm font-medium">Arrival documented</span>
                  <Badge variant="success" size="sm" className="w-fit">
                    {new Date(pacuRecord.arrival_time).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                  </Badge>
                ...
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm font-medium">Initial Aldrete</span>
                  <Badge
                    variant={(pacuRecord.initial_aldrete_score ?? 0) >= 8 ? 'success' : 'warning'}
                    size="sm"
                    className="w-fit"
                  >
                    {pacuRecord.initial_aldrete_score ?? 'N/A'}/10
                  </Badge>
                </div>
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm font-medium">Latest Aldrete</span>
                  <Badge
                    variant={(pacuRecord.latest_aldrete_score ?? 0) >= 9 ? 'success' : 'warning'}
                    size="sm"
                    className="w-fit"
                  >
                    {pacuRecord.latest_aldrete_score ?? 'N/A'}/10
                  </Badge>
                </div>
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm font-medium">Vitals recorded</span>
                  <Badge
                    variant={pacuRecord.vital_readings.length > 0 ? 'info' : 'outline'}
                    size="sm"
                    className="w-fit"
                  >
                    {pacuRecord.vital_readings.length}
                  </Badge>
                </div>
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <span className="text-sm font-medium">Complication flags</span>
                  <Badge
                    variant={pacuRecord.active_complication_count > 0 ? 'warning' : 'success'}
                    size="sm"
                    className="w-fit"
                  >
                    {pacuRecord.active_complication_count}
                  </Badge>
                </div>
                <div className="flex items-center justify-between rounded-lg border p-3">
                  ...
                  <Badge
                    variant={
                      pacuRecord.discharge_time
                        ? 'success'
                        : pacuRecord.ready_for_discharge
                          ? 'info'
                          : 'warning'
                    }
                    size="sm"
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  PACU Monitoring Trend
                  <HelpPopover content="Time-series chart of recovery vitals: heart rate, SpO2, Aldrete score, and pain score. Helps identify deterioration early." />
                </CardTitle>
              </CardHeader>
              <CardContent>
                {pacuRecord.vital_readings.length === 0 ? (
                  <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertTitle>No PACU vitals yet</AlertTitle>
                    <AlertDescription>Add the first recovery observation below.</AlertDescription>
                  </Alert>
                ) : (
          ...
          {pacuRecord.discharge_time ? null : pacuRecord.discharge_blockers.length > 0 ? (
            ...
            <Alert>
              ...
              <AlertDescription>
                The current recovery observations satisfy the discharge checklist. Complete the
                discharge form when transfer is confirmed.
              </AlertDescription>
            </Alert>
          )}
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  <HeartPulse className="h-4 w-4" />
                  Add PACU Vital
                  <HelpPopover content="Record a new recovery observation: heart rate, SpO2, blood pressure, Aldrete score (0-10), pain score, sedation level, and temperature." />
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Form {...vitalForm}>
                  <form className="space-y-4" onSubmit={vitalForm.handleSubmit(addVital)}>
                    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Recorded by user ID</FormLabel>
                            <FormControl>
                              <Input type="number" min={1} {...field} />
                            </FormControl>
                            <FormMessage />
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Aldrete score</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min={0}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Pain score</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                min={0}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Sedation level</FormLabel>
                            <FormControl>
                              <Input {...field} />
                            </FormControl>
                            <FormMessage />
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  Recovery Documentation & Handover
                  <HelpPopover content="Document post-operative complications (nausea, shivering, respiratory/cardiovascular issues), medications given in PACU, and the nursing handover for ward transfer." />
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Form {...documentationForm}>
                  <form
                    className="space-y-4"
                    onSubmit={documentationForm.handleSubmit(saveDocumentation)}
                  >
                    <div className="grid gap-3 sm:grid-cols-2">
                      {[
                        ...
                        <FormField
                          ...
                          name={name as keyof z.infer<typeof pacuDocumentationSchema>}
                          render={({ field }) => (
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Medications given in PACU</FormLabel>
                          <FormControl>
                            <Textarea rows={3} {...field} />
                          </FormControl>
                          <FormMessage />
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Handover recipient</FormLabel>
                            <FormControl>
                              <Input {...field} placeholder="Ward nurse, ICU nurse, or caregiver" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <div className="rounded-lg border p-3 text-sm text-muted-foreground">
                        ...
                        <p className="mt-1">
                          {pacuRecord.handover_completed_at
                            ? `Completed ${new Date(pacuRecord.handover_completed_at).toLocaleString()}`
                            : 'Not yet documented'}
                        </p>
                      </div>
                    </div>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
                <MoveRight className="h-4 w-4" />
                Discharge Workflow
                <HelpPopover content="Complete PACU discharge: record final Aldrete score, select discharge destination (ward, ICU, day case), and add discharge notes. Blocked until discharge criteria are met." />
              </CardTitle>
            </CardHeader>
            <CardContent>
              {pacuRecord.discharge_time ? (
                <Alert>
                  <CheckCircle2 className="h-4 w-4" />
                  <AlertTitle>Discharge already completed</AlertTitle>
                  <AlertDescription>
                    {pacuRecord.discharge_destination
                      ? `Destination: ${pacuRecord.discharge_destination.replace(/_/g, ' ')}`
                      : 'Recovery discharge has already been documented.'}
                  </AlertDescription>
                </Alert>
              ) : (
                <Form {...dischargeForm}>
                  <form
                    className="space-y-4"
                    onSubmit={dischargeForm.handleSubmit(dischargePatient)}
                  >
                    <div className="grid gap-4 sm:grid-cols-2">
                      <FormField
                        control={dischargeForm.control}
                        name="discharge_aldrete_score"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Discharge Aldrete score</FormLabel>
                            <FormControl>
                              <Input type="number" min={0} max={10} {...field} />
                            </FormControl>
                            <FormMessage />
                          ...
                          <FormItem>
                            ...
                            <Select value={field.value} onValueChange={field.onChange}>
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="DAY_CASE_DISCHARGE">
                                  Day Case Discharge
                                </SelectItem>
          ...
          {pacuRecord.discharge_time && pacuRecord.handover_given_to ? (
            <div className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
              ...
              <span className="font-medium text-foreground">{pacuRecord.handover_given_to}</span>{' '}
              with destination{' '}
              <span className="font-medium text-foreground">
                {pacuRecord.discharge_destination.replace(/_/g, ' ')}
              </span>
              .
            </div>
          ) : null}
