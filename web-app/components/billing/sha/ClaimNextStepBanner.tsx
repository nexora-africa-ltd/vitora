...
const SEVERITY_STYLES: Record<ClaimNextStep['severity'], string> = {
  ...
  warning:
    'border-amber-300/60 bg-amber-50 text-amber-900 dark:border-amber-800/60 dark:bg-amber-950/30 dark:text-amber-200',
    ...
    >
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          {step.description && (
            <p className="mt-0.5 text-xs opacity-90 sm:text-sm">{step.description}</p>
          )}
        </div>
      </div>
      {hasCta && (
