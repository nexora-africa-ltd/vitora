...
export function SHABenefitsAlert({ shaEligible, benefitsState, className }: SHABenefitsAlertProps) {
        ...
        <div className="flex gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 dark:border-amber-700 dark:bg-amber-950/40">
          ...
          <div className="min-w-0">
            ...
            {benefitsState.detail && benefitsState.detail !== benefitsState.message && (
              <p className="mt-1 break-words text-[11px] text-amber-700/90 dark:text-amber-300/90">
                {benefitsState.detail}
              </p>
            )}
            {(benefitsState.code || typeof benefitsState.upstreamStatus === 'number') && (
              <p className="mt-1 font-mono text-[10px] text-amber-700/80 dark:text-amber-300/80">
                {benefitsState.code || 'SHA_ERROR'}
                {typeof benefitsState.upstreamStatus === 'number'
                  ? ` (upstream ${benefitsState.upstreamStatus})`
                  : ''}
              </p>
            )}
          </div>
        ...
        <div className="flex gap-2 rounded-md border border-green-300 bg-green-50 p-3 dark:border-green-700 dark:bg-green-950/40">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-green-700 dark:text-green-400">
              This patient has active SHA benefit packages. SHA claims can be submitted for covered
              services.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2 rounded-md border border-red-300 bg-red-50 p-3 dark:border-red-700 dark:bg-red-950/40">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-red-700 dark:text-red-400">
              ...
              SHA claims cannot be submitted. Consider registering as a private/cash patient.
            </p>
          </div>
        </div>
      </div>
