    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {summary.pending_submission > 0 && (
          <div className="flex items-center justify-between rounded-md border border-amber-200 dark:border-amber-800/60 bg-amber-50 dark:bg-amber-900/20 px-3 py-2">
        ...
        {(action_items.time_bar_risk.length > 0 || action_items.queries.length > 0) && (
          <div className="space-y-2">
            ...
            {action_items.time_bar_risk.map((claim) => (
              <div
                key={claim.id}
                className="flex items-center gap-2 rounded-md border border-red-200 dark:border-red-800/60 bg-red-50 dark:bg-red-900/20 px-3 py-2"
              >
            ...
            {action_items.queries.map((claim) => (
              <div
                key={claim.id}
                className="flex items-center gap-2 rounded-md border border-orange-200 dark:border-orange-800/60 bg-orange-50 dark:bg-orange-900/20 px-3 py-2"
              >
                <MessageSquareWarning className="h-4 w-4 text-orange-600 shrink-0" />
                <span className="text-xs flex-1">
                  <span className="font-medium">{claim.claim_number}</span> — Query from SHA
                  <span className="text-muted-foreground ml-1">({claim.patient})</span>
                </span>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="flex items-center gap-2 rounded-md border p-2">
      ...
      <div className="min-w-0">
        ...
        <p className={`font-semibold ${isText ? 'text-xs' : 'text-sm'} truncate`}>
          {value}
        </p>
      </div>
    </div>
