...
export function ViewerToolbar({
  ...
  return (
    <div
      className={cn(
        'flex items-center gap-0.5 rounded-lg border bg-background/95 p-1 backdrop-blur supports-[backdrop-filter]:bg-background/60 sm:gap-1',
    ...
    >
      ...
      <Separator
        ...
        className={cn(isHorizontal ? 'h-6' : 'w-6', 'hidden sm:block')}
      />

      {/* Measurement Tools - hidden on mobile */}
      <div className={cn('hidden gap-0.5 sm:flex', !isHorizontal && 'flex-col')}>
        <ToolButton
          tool="ruler"
          activeTool={activeTool}
          onToolChange={onToolChange}
      ...
      <Separator
        orientation={isHorizontal ? 'vertical' : 'horizontal'}
        className={isHorizontal ? 'h-6' : 'w-6'}
      />
      ...
      <div className={cn('flex items-center gap-0.5 sm:gap-1', !isHorizontal && 'flex-col')}>
        ...
        />
        <span className="min-w-[40px] text-center text-xs text-muted-foreground sm:min-w-[60px]">
