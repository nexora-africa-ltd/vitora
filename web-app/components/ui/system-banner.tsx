...
const inlineColorClasses: Record<
  SemanticColor,
...
export default function SystemBanner({
    ...
    return (
      ...
      >
        <div className="flex gap-3">
          {icon && <div className={`mt-0.5 shrink-0 ${inlineStyles.icon}`}>{icon}</div>}
          <div className="flex-1 space-y-1.5">
            {text && (
              <p className={`text-sm font-medium leading-none sm:text-base ${inlineStyles.title}`}>
                {text}
              </p>
            )}
            {description && (
  ...
  return (
    <div
      className={`
        fixed left-0 top-0 z-50 flex h-0.5 w-full justify-center
        ${fixedBgClass}
      `}
      style={fixedBgStyle}
    >
      <span
        className={`
          absolute -bottom-4 max-w-[90vw] truncate rounded font-bold text-white shadow-md
          ${sizeClasses[size]}
          ${fixedBgClass}
        `}
        style={fixedBgStyle}
