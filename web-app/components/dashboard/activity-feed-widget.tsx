...
function getActionMeta(action: string): ActionMeta {
  const map: Record<string, ActionMeta> = {
    ...
    view_sensitive_patient: {
      icon: Shield,
      label: 'Accessed sensitive record',
  ...
  if (action.endsWith('_create'))
    return { icon: UserPlus, label: action.replace(/_/g, ' '), color: 'text-green-500' };
  if (action.endsWith('_update'))
    return { icon: Pencil, label: action.replace(/_/g, ' '), color: 'text-amber-500' };
  if (action.endsWith('_delete'))
    return { icon: Trash2, label: action.replace(/_/g, ' '), color: 'text-red-500' };
  if (action.endsWith('_view'))
    return { icon: Eye, label: action.replace(/_/g, ' '), color: 'text-blue-500' };
...
export function ActivityFeedWidget({ enabled = true }: { enabled?: boolean }) {
  const { data, isLoading } = useAuditLogs(enabled ? { page_size: MAX_ITEMS } : undefined);
        ...
        {entries.map((entry) => {
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs leading-relaxed">
                  ...
                  {resourceLabel && (
                    <Badge
                      variant="outline"
                      className="ml-1.5 inline-flex h-4 px-1 align-middle text-[9px]"
                    >
                      {resourceLabel}
                    </Badge>
                  )}
              ...
              <Icon className={`mt-1 h-3.5 w-3.5 shrink-0 ${meta.color}`} aria-hidden="true" />
            </li>
          );
        })}
      </ul>
