...
export function DispensingReport() {
  ...
  const filteredRecords = records.filter((record: DispensingReportRecord) => {
    const matchesDrug =
      !drugFilter || record.drug_name.toLowerCase().includes(drugFilter.toLowerCase());
    const matchesPatient =
      !patientFilter || record.patient_name.toLowerCase().includes(patientFilter.toLowerCase());
    return matchesDrug && matchesPatient;
  });

  // Calculate totals
  const totalDispensed = filteredRecords.reduce(
    (sum: number, r: DispensingReportRecord) => sum + r.quantity_dispensed,
    0
  );
  const totalValue = filteredRecords.reduce(
    (sum: number, r: DispensingReportRecord) => sum + parseFloat(r.total_cost),
    0
  );
    ...
    ? Object.entries(
        filteredRecords.reduce(
          (
            acc: Record<
              string,
              { drug_name: string; total_qty: number; total_cost: number; count: number }
            >,
            r: DispensingReportRecord
          ) => {
            if (!acc[r.drug_name]) {
      ...
      const csvContent = groupByDrug
        ? [
            ...
            ...(groupedData || []).map((item) =>
              [`"${item.drug_name}"`, item.total_qty, item.total_cost.toFixed(2), item.count].join(
                ','
              )
            ),
          ].join('\n')
        ...
        {totalPages > 1 && (
          <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-center text-sm text-muted-foreground sm:text-left">
              Showing {groupByDrug ? paginatedGroupedData.length : paginatedRecords.length} of{' '}
              {displayData.length} records (page {currentPage} of {totalPages})
            </p>
