...
const typeConfig: Record<
  ValidationType,
  { icon: typeof Shield; label: string; shortLabel: string }
> = {
...
export function ValidationStatusBadge({
  ...
  return (
    <Badge variant="outline" className={cn('w-fit shrink-0 gap-1', statusInfo.color, className)}>
...
export function ValidationSummary({
  ...
  const isFullyValidated = technicalStatus === 'APPROVED' && clinicalStatus === 'APPROVED';
  ...
  if (isFullyValidated) {
    return (
      <Badge
        variant="outline"
        className={cn(
          'w-fit shrink-0 gap-1 border-green-200 bg-green-100 text-green-800 dark:border-green-800 dark:bg-green-900/30 dark:text-green-300',
          className
        )}
      >
