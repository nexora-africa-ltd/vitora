...
export function QuickCheckinDialog({
  ...
  const shouldSkipTriage =
    VISIT_REASON_OPTIONS.find((o) => o.value === visitReason)?.skipTriage ?? false;
  ...
  const canSubmit =
    isReferralValid && isChronicCareValid && (!isClinicMode || resolvedClinicId !== undefined);
  ...
  const handleCheckin = async () => {
    ...
    const referralNote =
      isReferral && referralFacility.trim() ? `Referred from: ${referralFacility.trim()}` : '';
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="flex h-[calc(100dvh-1rem)] max-h-[calc(100dvh-1rem)] flex-col gap-0 overflow-hidden p-0 sm:h-[min(90vh,44rem)] sm:max-h-[90vh] sm:max-w-[680px]">
          <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border-0 bg-background">
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-x-0 top-0 h-24 bg-[radial-gradient(circle_at_top_left,rgba(20,184,166,0.12),transparent_55%),radial-gradient(circle_at_top_right,rgba(14,165,233,0.1),transparent_50%)] sm:h-40"
            />
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-2">
                  ...
                  <DialogDescription className="sr-only">
                    Review the visit reason, choose triage or a clinic destination, then confirm the
                    patient check-in.
                  </DialogDescription>
                ...
                {suggestedType && (
                  <Alert className="border-primary/20 bg-primary/5">
                    <AlertTitle className="flex items-center gap-2 text-xs sm:text-sm">
                      ...
                      <Badge
                        variant={suggestedType === 'NEW' ? 'default' : 'secondary'}
                      ...
                      <div className="space-y-2">
                        ...
                        {!referralFacility.trim() && (
                          <p className="text-xs text-destructive">
                            Referral facility name is required.
                          </p>
                        )}
                      </div>
                    )}
                        ...
                        >
                          <div className="flex items-center gap-2 text-sm font-medium">
                            <Building2
                              className={cn(
                                'h-4 w-4',
                                isClinicMode ? 'text-success-foreground' : 'text-primary'
                              )}
                            />
                            Direct to Clinic
                            {isClinicMode && (
                          ...
                          <p
                            className={cn(
                              'mt-2 text-xs',
                              isClinicMode ? 'text-success-foreground/90' : 'text-muted-foreground'
                            )}
                          >
                            Skip triage when the visit reason or workflow already determines the
                            destination.
                          </p>
                        </button>
                      </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            ...
                            <Input
                              ...
                              placeholder={
                                isChronicCare
                                  ? 'Search chronic care clinics...'
                                  : 'Search clinics by name or type...'
                              }
                              className="pl-9"
                            />
                          </div>
