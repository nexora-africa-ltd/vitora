...
const INLINE_BRACKET_PATTERN =
  /\[([^\]]*(?:AI|suggested|clinician|verify|review|edit|sign|not documented)[^\]]*)\]/gi;
/** Matches italic advisory/meta lines (e.g. "*Relevant guideline context...*"). */
const ITALIC_ADVISORY_PATTERN =
  /^\*.*(?:guideline|context|reference|advisory|note|disclaimer|AI.generated|clinician.review).*\*?\s*$/i;
...
const HEADER_SECTION_KEYS = new Set(['patient_demographics', 'admission_details']);
...
export function buildTemplateAlignedContent(
  ...
  for (const tplSection of templateSections) {
    ...
    if (content) {
      const consumePatterns = KEY_TO_TITLE_PATTERNS[tplSection.key] || [
        tplSection.label.toLowerCase(),
      ];
      for (const s of formSections) {
        if (
          !usedFormSectionIds.has(s.id) &&
          consumePatterns.some((p) => fuzzyTitleMatch(s.title, p))
        ) {
 ...
 */
export function splitWrapperSection(section: {
  section_id: string;
  title: string;
  content: string;
  ...
  for (const line of lines) {
    ...
    const heading =
      h2Match?.[1]?.trim() || (!isNumberedBullet && isHeadingLike ? boldHeading : '') || '';
...
export function formatAdvisoryText(text: string): React.ReactNode {
  ...
  return (
    <>
      <em className="font-medium">{match[1]}</em>
      {match[2] ? ` ${match[2]}` : ''}
    </>
  );
}
...
export function extractFollowUpDate(text: string): string | null {
  ...
  const namedMatch =
    text.match(new RegExp(`\\b(${months})\\s+(\\d{1,2})(?:st|nd|rd|th)?,?\\s+(\\d{4})\\b`, 'i')) ||
  ...
  const relMatch =
    text.match(/\b(?:in|after|within)\s+(\d+)\s*(day|week|month)s?\b/i) ||
 ...
 */
export function parseMedicationLines(
  lines: string[]
  ...
  const isSkippable = (line: string) => {
    ...
    if (
      /^(primary|secondary|discharge\s*diagnosis|condition\s*at|follow[\s-]?up|hospital\s*course|history|clinical\s*presentation|the\s+clinical)/i.test(
        t
      )
    )
      return true;
