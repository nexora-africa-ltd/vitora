...
export const locationsApi = {
  ...
  async getSubCounties(countyId: number): Promise<SubCounty[]> {
    const response = await apiClient.get<SubCounty[]>(`/api/locations/sub-counties/`, {
      params: { county: countyId },
    });
    return parseResponse(SubCountyArraySchema, response.data, {
      context: 'locationsApi.getSubCounties',
    });
  },
  ...
  async getWards(subCountyId: number): Promise<Ward[]> {
    const response = await apiClient.get<Ward[]>(`/api/locations/wards/`, {
      params: { sub_county: subCountyId },
    });
    return parseResponse(LocationWardArraySchema, response.data, {
      context: 'locationsApi.getWards',
    });
  },
};
