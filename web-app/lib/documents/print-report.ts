...
<div class="report">
  ...
  <div class="footer">
    <div class="footer-notes">
      <p>Generated by {{system.name}}. {{system.signature_note}}</p>
    </div>
    <div class="qr">QR</div>
  </div>
</div>
...
function buildTemplateData(data: PrintRadiologyReportData): Record<string, unknown> {
  ...
  const criticalDisplay = report.is_critical ? 'display: block;' : 'display: none;';
  const techniqueDisplay = report.technique ? 'display: block;' : 'display: none;';
  const comparisonDisplay = report.comparison ? 'display: block;' : 'display: none;';
  const recommendationsDisplay = report.recommendations ? 'display: block;' : 'display: none;';
  const amendmentsDisplay = report.amendment_count > 0 ? 'display: block;' : 'display: none;';
  ...
  return {
    ...
    system: {
      ...
      signature_note: hasDigitalSignature ? 'Digitally signed.' : 'Not digitally signed.',
    },
  };
}
