...
export const vitalsSchema = z
  .object({
    ...
    spo2: nullableNumber(VITAL_RANGES.spo2.min, VITAL_RANGES.spo2.max, 'SpO₂'),
    weight: nullableNumber(VITAL_RANGES.weight.min, VITAL_RANGES.weight.max, 'Weight'),
    height: nullableNumber(VITAL_RANGES.height.min, VITAL_RANGES.height.max, 'Height'),
  })
  .refine(
    (data) => {
...
export const triageVitalsSchema = z.object({
  temperature: z
    .union([z.literal(null), z.number().min(30, 'Must be 30-45°C').max(45, 'Must be 30-45°C')])
    .nullable()
    .optional(),
  heart_rate: z
    .union([z.literal(null), z.number().min(0, 'Must be 0-300').max(300, 'Must be 0-300')])
    .nullable()
    .optional(),
  systolic_bp: z
    .union([z.literal(null), z.number().min(0, 'Must be 0-300').max(300, 'Must be 0-300')])
    ...
    .optional(),
  diastolic_bp: z
    .union([z.literal(null), z.number().min(0, 'Must be 0-200').max(200, 'Must be 0-200')])
    .nullable()
    .optional(),
  spo2: z
    .union([z.literal(null), z.number().min(0, 'Must be 0-100%').max(100, 'Must be 0-100%')])
    .nullable()
    .optional(),
  respiratory_rate: z
    .union([z.literal(null), z.number().min(0, 'Must be 0-60').max(60, 'Must be 0-60')])
    .nullable()
    .optional(),
  weight: z
    .union([z.literal(null), z.number().min(0, 'Must be positive').max(500, 'Must be < 500kg')])
    .nullable()
    .optional(),
  height: z
    .union([z.literal(null), z.number().min(0, 'Must be positive').max(300, 'Must be < 300cm')])
    .nullable()
    .optional(),
});
