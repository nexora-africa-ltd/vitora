...
describe('counsellingKeys', () => {
  it('builds stable query keys', () => {
    ...
    expect(counsellingKeys.referralSessions(3)).toEqual(['counselling', 'sessions', 'referral', 3]);
  });
});
...
describe('counselling query hooks', () => {
  ...
  it('fetches types, referrals, and sessions', async () => {
    ...
    mockCounsellingApi.listReferrals.mockResolvedValueOnce({
      count: 1,
    ...
    mockCounsellingApi.listSessions.mockResolvedValueOnce({
      count: 1,
      results: [mockSession],
    } as never);
    ...
    const types = renderHook(() => useCounsellingTypes({ is_active: true } as never), {
    ...
    const referrals = renderHook(() => useCounsellingReferrals({ status: 'PENDING' as never }), {
      wrapper: createWrapper().wrapper,
    });
    ...
    const referral = renderHook(() => useCounsellingReferral(3), {
      wrapper: createWrapper().wrapper,
    });
    ...
    const referralByNumber = renderHook(
      () => useCounsellingReferralByNumber('COUN-20260315-0001'),
      { wrapper: createWrapper().wrapper }
    );
    ...
    const sessions = renderHook(() => useCounsellingSessions({ status: 'SCHEDULED' as never }), {
      wrapper: createWrapper().wrapper,
    });
    ...
    const session = renderHook(() => useCounsellingSession(5), {
      wrapper: createWrapper().wrapper,
    });
    ...
    const referralSessions = renderHook(() => useCounsellingReferralSessions(3), {
      wrapper: createWrapper().wrapper,
    });
  ...
  it('does not fetch disabled detail queries', () => {
    ...
    renderHook(() => useCounsellingReferralByNumber(undefined), {
      wrapper: createWrapper().wrapper,
    });
    renderHook(() => useCounsellingSession(undefined), { wrapper: createWrapper().wrapper });
    renderHook(() => useCounsellingReferralSessions(undefined), {
      wrapper: createWrapper().wrapper,
    });
...
describe('counselling mutation hooks', () => {
  ...
  it('invalidates referral queries for referral mutations', async () => {
    const cases = [
      {
        useHook: useCreateCounsellingReferral,
        apiMock: mockCounsellingApi.createReferral,
        input: {
          patient_id: 1,
          encounter_id: 100,
          counselling_type: 'INDIVIDUAL',
        ...
        calledWith: [
          { patient_id: 1, encounter_id: 100, counselling_type: 'INDIVIDUAL', reason: 'ANXIETY' },
        ],
        resolved: mockReferral,
        keys: [counsellingKeys.referrals()],
      },
      {
        useHook: useUpdateCounsellingReferral,
        apiMock: mockCounsellingApi.updateReferral,
        input: { id: 3, data: { urgency: 'HIGH' } },
        calledWith: [3, { urgency: 'HIGH' }],
        resolved: mockReferral,
        keys: [counsellingKeys.referral(3), counsellingKeys.referrals()],
      },
      {
        useHook: useDeleteCounsellingReferral,
        apiMock: mockCounsellingApi.deleteReferral,
        input: 3,
        calledWith: [3],
        resolved: undefined,
        keys: [counsellingKeys.referrals()],
      ...
      {
        useHook: useAcceptCounsellingReferral,
        apiMock: mockCounsellingApi.acceptReferral,
        input: 3,
        calledWith: [3],
      ...
      {
        useHook: useRejectCounsellingReferral,
        apiMock: mockCounsellingApi.rejectReferral,
        input: { id: 3, reason: 'Insufficient details' },
        calledWith: [3, 'Insufficient details'],
        resolved: { ...mockReferral, status: 'DECLINED' },
        keys: [counsellingKeys.referral(3), counsellingKeys.referrals()],
      },
      {
        useHook: useAssignCounsellor,
        apiMock: mockCounsellingApi.assignCounsellor,
        input: { id: 3, counsellorId: 12 },
        calledWith: [3, 12],
        resolved: mockReferral,
        keys: [counsellingKeys.referral(3), counsellingKeys.referrals()],
      },
      {
        useHook: useGenerateCounsellingSessions,
        apiMock: mockCounsellingApi.generateSessions,
        input: { id: 3, count: 4 },
        calledWith: [3, 4],
        resolved: [mockSession],
        keys: [
          counsellingKeys.referral(3),
          counsellingKeys.referralSessions(3),
          counsellingKeys.sessions(),
        ],
      },
      {
        useHook: useStartCounsellingReferral,
        apiMock: mockCounsellingApi.startReferral,
        input: 3,
        calledWith: [3],
        resolved: { ...mockReferral, status: 'IN_PROGRESS' },
        keys: [counsellingKeys.referral(3), counsellingKeys.referrals()],
      },
      {
        useHook: useCompleteCounsellingReferral,
        apiMock: mockCounsellingApi.completeReferral,
        input: 3,
        calledWith: [3],
        resolved: { ...mockReferral, status: 'COMPLETED' },
        keys: [counsellingKeys.referral(3), counsellingKeys.referrals()],
      },
      {
        useHook: useCancelCounsellingReferral,
        apiMock: mockCounsellingApi.cancelReferral,
        input: { id: 3, reason: 'Patient deferred' },
        calledWith: [3, 'Patient deferred'],
        resolved: { ...mockReferral, status: 'CANCELLED' },
        keys: [counsellingKeys.referral(3), counsellingKeys.referrals()],
      },
    ];
  ...
  it('invalidates session queries for session mutations', async () => {
    const cases = [
      {
        useHook: useCreateCounsellingSession,
        apiMock: mockCounsellingApi.createSession,
        input: { referral: 3, scheduled_date: '2026-03-16' },
        calledWith: [{ referral: 3, scheduled_date: '2026-03-16' }],
        resolved: mockSession,
        keys: [counsellingKeys.sessions(), counsellingKeys.referralSessions(3)],
      },
      {
        useHook: useStartCounsellingSession,
        apiMock: mockCounsellingApi.startSession,
        input: 5,
        calledWith: [5],
        resolved: { ...mockSession, status: 'IN_PROGRESS' },
        keys: [counsellingKeys.session(5), counsellingKeys.sessions()],
      },
      {
        useHook: useCompleteCounsellingSession,
        apiMock: mockCounsellingApi.completeSession,
        input: { id: 5, data: { progress_notes: 'Improved coping' } },
        calledWith: [5, { progress_notes: 'Improved coping' }],
        resolved: { ...mockSession, status: 'COMPLETED', referral: 3 },
        keys: [counsellingKeys.session(5), counsellingKeys.sessions(), counsellingKeys.referral(3)],
      },
      {
        useHook: useCancelCounsellingSession,
        apiMock: mockCounsellingApi.cancelSession,
        input: { id: 5, reason: 'Scheduling conflict' },
        calledWith: [5, 'Scheduling conflict'],
        resolved: { ...mockSession, status: 'CANCELLED' },
        keys: [counsellingKeys.session(5), counsellingKeys.sessions()],
      },
      {
        useHook: useMarkCounsellingSessionNoShow,
        apiMock: mockCounsellingApi.markNoShow,
        input: 5,
        calledWith: [5],
      ...
      {
        useHook: useRescheduleCounsellingSession,
        apiMock: mockCounsellingApi.rescheduleSession,
        input: { id: 5, newDate: '2026-03-22', newTime: '14:00:00' },
        calledWith: [5, '2026-03-22', '14:00:00'],
        resolved: { ...mockSession, scheduled_date: '2026-03-22' },
        keys: [counsellingKeys.session(5), counsellingKeys.sessions()],
      },
    ];
