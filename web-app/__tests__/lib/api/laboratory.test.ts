...
describe('laboratoryApi', () => {
  ...
  it('covers primary GET endpoints and array or paginated response fallbacks', async () => {
    const getCases = [
      {
        fn: () => laboratoryApi.listTests({ search: 'cbc' } as never),
        args: ['/api/lab/tests/', { params: { search: 'cbc' } }],
      },
      { fn: () => laboratoryApi.getTest('CBC'), args: ['/api/lab/tests/CBC/'] },
      {
        fn: () => laboratoryApi.searchTests('cbc'),
        args: ['/api/lab/tests/', { params: { search: 'cbc', is_active: true } }],
        resultFromResults: true,
      },
      {
        fn: () => laboratoryApi.listOrders({ status: 'PENDING' } as never),
        args: ['/api/lab/orders/', { params: { status: 'PENDING' } }],
      },
      ...
      {
        fn: () => laboratoryApi.getTurnaroundTimeReport('2026-03-01', '2026-03-15'),
        args: [
      ...
      {
        fn: () => laboratoryApi.getWorkloadReport('2026-03-01', '2026-03-15'),
        args: [
          '/api/lab/reports/workload/',
          { params: { start: '2026-03-01', end: '2026-03-15' } },
        ],
      },
      {
        fn: () => laboratoryApi.getCriticalValuesReport('2026-03-01', '2026-03-15'),
        args: [
      ...
      {
        fn: () => laboratoryApi.getSampleRejectionReport('2026-03-01', '2026-03-15'),
        args: [
          '/api/lab/reports/rejections/',
          { params: { start: '2026-03-01', end: '2026-03-15' } },
        ],
      },
      {
        fn: () => laboratoryApi.lookupByBarcode('BC1'),
        args: ['/api/lab/queue/lookup/', { params: { barcode: 'BC1' } }],
      },
    ...
    expect(await laboratoryApi.listDiagnosticReports({ status: 'FINAL' as never })).toEqual([
      { id: 10 },
    ]);
  });
  ...
  it('covers POST, PATCH, DELETE, upload fallback, and PDF download flows', async () => {
    const postPatchCases = [
      {
        fn: () => laboratoryApi.createOrder({ patient: 1 } as never),
        method: 'post',
        args: ['/api/lab/orders/', { patient: 1 }],
      },
      {
        fn: () => laboratoryApi.updateOrder('LAB-1', { status: 'DRAFT' } as never),
        method: 'patch',
        args: ['/api/lab/orders/LAB-1/', { status: 'DRAFT' }],
      },
      {
        fn: () => laboratoryApi.submitOrder('LAB-1'),
        method: 'post',
        args: ['/api/lab/orders/LAB-1/submit/'],
      },
      {
        fn: () => laboratoryApi.collectSpecimen('LAB-1', 'S1'),
        method: 'post',
        args: ['/api/lab/orders/LAB-1/collect-specimen/', { sample_id: 'S1' }],
      },
      {
        fn: () => laboratoryApi.cancelOrder('LAB-1', 'bad'),
        method: 'post',
        args: ['/api/lab/orders/LAB-1/cancel/', { reason: 'bad' }],
      },
      {
        fn: () => laboratoryApi.addOrderItem('LAB-1', 2, 'urgent'),
        method: 'post',
        args: ['/api/lab/orders/LAB-1/items/', { test: 2, special_instructions: 'urgent' }],
      },
      {
        fn: () => laboratoryApi.addResult('LAB-1', { result_value: '5.0' } as never),
        method: 'post',
        args: ['/api/lab/orders/LAB-1/results/', { result_value: '5.0' }],
      },
      {
        fn: () => laboratoryApi.updateResult(1, { result_value: '6.0' } as never),
        method: 'patch',
        args: ['/api/lab/results/1/', { result_value: '6.0' }],
      },
      {
      ...
      {
        fn: () =>
          laboratoryApi.createResultValidation(1, { validation_type: 'TECHNICAL' } as never),
        method: 'post',
        args: ['/api/lab/results/1/validate/', { validation_type: 'TECHNICAL' }],
      },
      {
        fn: () => laboratoryApi.createInstrument({ name: 'Analyzer' } as never),
        method: 'post',
        args: ['/api/lab/instruments/', { name: 'Analyzer' }],
      },
      {
        fn: () => laboratoryApi.updateInstrument(1, { name: 'Updated' } as never),
        method: 'patch',
        args: ['/api/lab/instruments/1/', { name: 'Updated' }],
      },
      {
        fn: () => laboratoryApi.seedDriverTemplates(),
        method: 'post',
        args: ['/api/lab/analyzers/templates/seed_defaults/'],
      },
      {
        fn: () => laboratoryApi.markAnalyzerRunError(1, 'Oops'),
        method: 'post',
        args: ['/api/lab/analyzer-runs/1/mark_error/', { error_message: 'Oops' }],
      },
      {
        fn: () => laboratoryApi.createDiagnosticReport({ lab_order: 1 } as never),
        method: 'post',
        args: ['/api/lab/diagnostic-reports/', { lab_order: 1 }],
      },
      {
        fn: () => laboratoryApi.updateDiagnosticReport(1, { conclusion: 'Fine' } as never),
        method: 'patch',
        args: ['/api/lab/diagnostic-reports/1/', { conclusion: 'Fine' }],
      },
      {
        fn: () => laboratoryApi.finalizeDiagnosticReport(1),
        method: 'post',
        args: ['/api/lab/diagnostic-reports/1/finalize/'],
      },
      {
        fn: () => laboratoryApi.amendDiagnosticReport(1, 'Updated'),
        method: 'post',
        args: ['/api/lab/diagnostic-reports/1/amend/', { conclusion: 'Updated' }],
      },
      {
        fn: () => laboratoryApi.cancelDiagnosticReport(1, 'Bad'),
        method: 'post',
        args: ['/api/lab/diagnostic-reports/1/cancel/', { reason: 'Bad' }],
      },
      {
        fn: () => laboratoryApi.collectSample('Q1', 'S1'),
        method: 'post',
        args: ['/api/lab/queue/Q1/collect/', { sample_id: 'S1' }],
      },
      {
        fn: () => laboratoryApi.assignQueueEntry('Q1', 2),
        method: 'post',
        args: ['/api/lab/queue/Q1/assign/', { technician_id: 2 }],
      },
      {
        fn: () => laboratoryApi.startProcessing('Q1'),
        method: 'post',
        args: ['/api/lab/queue/Q1/start-processing/'],
      },
      {
        fn: () => laboratoryApi.submitForReview('Q1'),
        method: 'post',
        args: ['/api/lab/queue/Q1/submit-review/'],
      },
      {
        fn: () => laboratoryApi.releaseResults('Q1'),
        method: 'post',
        args: ['/api/lab/queue/Q1/release/'],
      },
      {
        fn: () => laboratoryApi.rejectSample('Q1', 'bad sample'),
        method: 'post',
        args: ['/api/lab/queue/Q1/reject/', { reason: 'bad sample' }],
      },
      {
        fn: () => laboratoryApi.updateNotes('Q1', 'done', true),
        method: 'post',
        args: ['/api/lab/queue/Q1/notes/', { notes: 'done', append: true }],
      },
    ];
    ...
    expect(mockApiClient.post).toHaveBeenLastCalledWith(
      '/api/lab/diagnostic-reports/1/generate_pdf/'
    );
    expect(mockApiClient.get).toHaveBeenLastCalledWith('/files/report.pdf', {
      responseType: 'blob',
    });
    expect(blob).toBeInstanceOf(Blob);

    mockApiClient.post.mockResolvedValueOnce({ data: { pdf_url: null } });
    await expect(laboratoryApi.downloadDiagnosticReportPdf(2)).rejects.toThrow(
      'Diagnostic report PDF URL was not returned.'
    );
  });
