...
test.describe('Stock Alerts - Panel View', () => {
  ...
  test('should show unresolved alert count badge', async ({ page }) => {
    ...
    await expect(
      alertsTab.getByText(/\d+/).or(alertsTab.locator('[data-testid="alert-count"]'))
    ).toBeVisible();
  });
  ...
  test('should display alert list', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="alerts-panel"], [data-testid="alert-list"]', {
      timeout: 10000,
    });
  ...
  test('should display alert message', async ({ page }) => {
    ...
    await expect(
      alertList
        .getByText(/expires.in/i)
        .first()
        .or(alertList.getByText(/37.days/i).first())
    ).toBeVisible();
  });
...
test.describe('Stock Alerts - Filtering', () => {
  ...
  test('should filter by alert type', async ({ page }) => {
    const typeFilter = page
      ...
      .or(page.getByTestId('alert-type-filter'));
  ...
  test('should filter by severity', async ({ page }) => {
    const severityFilter = page
      ...
      .or(page.getByTestId('severity-filter'));

    await expect(severityFilter).toBeVisible();
  });

  test('should toggle resolved/unresolved alerts', async ({ page }) => {
    const resolvedToggle = page
      .getByRole('checkbox', { name: /resolved|show.resolved/i })
      .or(page.getByRole('switch', { name: /resolved/i }))
      .or(page.getByTestId('resolved-toggle'));
  ...
  test('should have quick filter for low stock alerts', async ({ page }) => {
    const lowStockFilter = page
      ...
      .or(page.getByTestId('low-stock-filter'));
  ...
  test('should have quick filter for expiring alerts', async ({ page }) => {
    const expiringFilter = page
      .getByRole('button', { name: /expiring/i })
  ...
  test('should filter to show only critical alerts', async ({ page }) => {
    ...
    const severityFilter = page
      ...
      .first()
      .or(page.getByTestId('severity-filter'))
      .or(page.getByRole('button', { name: /severity|critical|high|medium|low/i }).first());
...
test.describe('Stock Alerts - Acknowledge', () => {
  ...
  test('should have acknowledge button for unacknowledged alerts', async ({ page }) => {
    const unacknowledgedAlert = page
      .locator('[data-testid="alert-item"]')
      .filter({ hasText: /out.of.stock/i })
      .first();

    const acknowledgeButton = unacknowledgedAlert
      .getByRole('button', { name: /acknowledge|ack/i })
      .or(unacknowledgedAlert.getByTestId('acknowledge-button'));
  ...
  test('should show acknowledged by user', async ({ page }) => {
    ...
    const acknowledgedIndicator = page
      .getByText(/acknowledged/i)
      .first()
      .or(
        page
          .locator('[data-testid="alert-item"]')
  ...
  test('should show acknowledged timestamp', async ({ page }) => {
    ...
    const acknowledgedWithTime = page
      .getByText(/acknowledged.*\d|\d.*acknowledged/i)
      .first()
      .or(
        page
          .locator('[data-testid="alert-item"]')
...
test.describe('Stock Alerts - Resolve', () => {
  ...
  test('should have resolve button for alerts', async ({ page }) => {
    ...
    const resolveButton = alert
      .getByRole('button', { name: /resolve/i })
      .or(alert.getByTestId('resolve-button'));
...
test.describe('Stock Alerts - Navigation', () => {
  ...
  test('should navigate to drug when clicking drug name', async ({ page }) => {
    const drugLink = page.getByRole('link', { name: /metformin/i }).or(
      page
        .locator('[data-testid="alert-item"]')
        .filter({ hasText: /metformin/i })
        .getByRole('button', { name: /view.drug/i })
    );
  ...
  test('should have quick action to reorder stock for low stock alerts', async ({ page }) => {
    const lowStockAlert = page
      .locator('[data-testid="alert-item"]')
      .filter({ hasText: /low.stock|out.of.stock/i })
      .first();

    const reorderButton = lowStockAlert
      .getByRole('button', { name: /reorder|order/i })
      .or(lowStockAlert.getByTestId('reorder-button'));
...
test.describe('Stock Alerts - Dashboard Widget', () => {
  ...
  test('should show critical alerts on pharmacy dashboard', async ({ page }) => {
    ...
    const alertsWidget = page
      ...
      .or(page.locator('.alerts-summary'));
  ...
  test('should show alert count summary', async ({ page }) => {
    ...
    const totalBadge = alertsWidget
      .locator('.text-lg')
      .getByRole('status')
      .or(alertsWidget.getByText(/^\d+$/).first());
...
test.describe('Stock Alerts - Auto-Generation', () => {
  ...
  test('should indicate auto-generated alerts', async ({ page }) => {
    ...
    const alertList = page.getByTestId('alert-list').or(page.getByTestId('alerts-panel'));
    const systemBadge = alertList
      .getByText(/system|auto|generated/i)
      .first()
      .or(page.getByTestId('system-generated-badge'));
  ...
  test('should have refresh alerts button', async ({ page }) => {
    ...
    const refreshButton = page
      .getByRole('button', { name: /refresh|regenerate/i })
      .or(page.getByTestId('refresh-alerts-button'));
...
test.describe('Stock Alerts - Settings', () => {
  ...
  test('should have alert settings button', async ({ page }) => {
    const settingsButton = page
      ...
      .or(page.getByTestId('alert-settings-button'));
