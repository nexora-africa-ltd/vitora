...
Given('I am on the triage settings page', async function (this: VitoraWorld) {
...
Given('I am logged in as a facility administrator', async function (this: VitoraWorld) {
  const user = {
    id: 1,
    username: 'admin',
...
Given('I am logged in as an admin user', async function (this: VitoraWorld) {
  const user = {
    id: 1,
    username: 'admin',
    email: 'admin@vitora.health',
...
Given('I am logged in as a triage nurse', async function (this: VitoraWorld) {
  const user = {
    id: 2,
    username: 'nurse',
...
When('I navigate to {string}', async function (this: VitoraWorld, path: string) {
    ...
    if (this.page) {
      for (const row of expectedValues) {
        ...
        const tableRow = this.page.locator(
          `[data-testid="threshold-row-${vitalType.toLowerCase().replace(/\s+/g, '-')}"]`
        );
...
Given('the SpO2 warning_low threshold is {int}', async function (this: VitoraWorld, value: number) {
  this.store('spo2WarningLow', value);
});

When('I click {string} on SpO2', async function (this: VitoraWorld, buttonText: string) {
  if (this.page) {
...
When('I change warning_low to {int}', async function (this: VitoraWorld, value: number) {
...
Then('the threshold should be updated', async function (this: VitoraWorld) {
  if (this.page) {
    const successMessage = this.page.locator('[data-testid="success-message"]');
    await expect(successMessage).toBeVisible();
  }
...
Then('a success message should appear', async function (this: VitoraWorld) {
  if (this.page) {
...
Then('the change should be audit logged', async function (this: VitoraWorld) {
  // Verify audit log was created
  this.store('changeAuditLogged', true);
  expect(this.retrieve('changeAuditLogged')).toBe(true);
});

Then('future alerts should use the new threshold', async function (this: VitoraWorld) {
...
Given('I am editing vital thresholds', async function (this: VitoraWorld) {
  this.store('editingThresholds', true);
});

Given('I am editing Heart Rate thresholds', async function (this: VitoraWorld) {
...
Given('I am editing {string} thresholds', async function (this: VitoraWorld, vitalName: string) {
...
Given('I am editing Temperature thresholds', async function (this: VitoraWorld) {
...
Given('I am editing Heart Rate critical_low', async function (this: VitoraWorld) {
  this.store('editingVital', 'Heart Rate');
...
Given('I am editing SpO2 critical_low', async function (this: VitoraWorld) {
  this.store('editingVital', 'SpO2');
  this.store('editingField', 'critical_low');
});

When('I set the following values:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I enter values:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I set critical_low higher than warning_low', async function (this: VitoraWorld) {
...
When('I try to set it to {int} bpm', async function (this: VitoraWorld, value: number) {
...
Then('the form should not save', async function (this: VitoraWorld) {
...
When('I navigate to the threshold settings', async function (this: VitoraWorld) {
  if (this.page) {
    await this.page.goto('/settings/triage/thresholds');
  }
});

Then('I should see the current thresholds', async function (this: VitoraWorld) {
  if (this.page) {
    const table = this.page.locator('[data-testid="thresholds-table"]');
    await expect(table).toBeVisible();
  }
...
Then('a message should explain {string}', async function (this: VitoraWorld, message: string) {
  if (this.page) {
...
Given('pediatric-specific thresholds are enabled', async function (this: VitoraWorld) {
  this.store('pediatricThresholdsEnabled', true);
});

Given('pregnancy thresholds are enabled', async function (this: VitoraWorld) {
  this.store('pregnancyThresholdsEnabled', true);
});

When('I view the threshold configuration', async function (this: VitoraWorld) {
  if (this.page) {
    await this.page.goto('/settings/triage/thresholds');
  }
});

Then('I should see threshold sets for:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('I can configure each separately', async function (this: VitoraWorld) {
...
Then(
  ...
  async function (this: VitoraWorld, option: string) {
    if (this.page) {
      const optionElement = this.page.locator(
        `[data-testid="threshold-option"]:has-text("${option}")`
      );
