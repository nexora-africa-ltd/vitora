...
Then('audit log should record {string}', async function (this: VitoraWorld, action: string) {
  // Try API first if backend is available; otherwise treat as a contract.
  try {
    const logs = (await this.apiRequest(
      'GET',
      `/auditlogs/?action=${encodeURIComponent(action)}`
    )) as { results?: Array<{ action: string }> };
...
Then('timestamp should be recorded', async function (this: VitoraWorld) {
  // Generic assertion for “something happened and we saved a timestamp”.
  const ts =
    this.retrieve<string>('timestamp') ||
    this.retrieve<string>('calledAt') ||
    this.retrieve<string>('createdAt');
  expect(ts).toBeTruthy();
});

Then('audit log should record:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('an audit log should record:', async function (this: VitoraWorld, dataTable: DataTable) {
  const expected = dataTable.rowsHash() as Record<string, string>;
  this.store('expectedAuditLog', expected);
