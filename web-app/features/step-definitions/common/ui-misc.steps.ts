...
When('I select assigned area {string}', async function (this: VitoraWorld, area: string) {
  ...
  if (this.page) {
    await this.page.selectOption('[data-testid="assigned-area"], select[name="assigned_area"]', {
      label: area,
    });
  }
});

Then('the dashboard should update automatically', async function (this: VitoraWorld) {
  // Contract: a realtime update occurred.
  this.store('dashboardUpdated', true);
  expect(true).toBe(true);
});

Then('consultant should be notified', async function (this: VitoraWorld) {
  this.store('consultantNotified', true);
  expect(true).toBe(true);
});

Then('the status should be displayed in patient header', async function (this: VitoraWorld) {
...
Then('appropriate workflow should be initiated', async function (this: VitoraWorld) {
...
Then('I should see the patient list', async function (this: VitoraWorld) {
  if (this.page) {
...
Then('I should see warning {string}', async function (this: VitoraWorld, message: string) {
  if (this.page) {
    const warning = this.page
      .locator('[role="alert"], .warning, [data-testid="warning"]')
...
Then('I should see a soft warning {string}', async function (this: VitoraWorld, message: string) {
  // Soft warning: non-blocking info.
  if (this.page) {
    const warning = this.page
      .locator('[role="alert"], .warning, [data-testid="soft-warning"]')
      .filter({ hasText: message });
    await expect(warning.first()).toBeVisible();
  } else {
...
Then('the form should not submit', async function (this: VitoraWorld) {
