...
Given('patient is in queue', async function (this: VitoraWorld) {
...
When('I view the queue dashboard', async function (this: VitoraWorld) {
...
Given('I am viewing the queue dashboard', async function (this: VitoraWorld) {
  this.currentPage = 'queue dashboard';
  if (this.page) {
    await this.page.goto('/queue/dashboard');
    await this.page.waitForLoadState('networkidle');
...
When('I view the queue', async function (this: VitoraWorld) {
...
When('I add patient to queue', async function (this: VitoraWorld) {
  const patientName = `${this.patient?.firstName || 'Jane'} ${this.patient?.lastName || 'Wanjiku'}`;
  const mrn = this.patient?.mrn;
  const entries = getQueue(this);
  entries.push({ patientName, mrn, department: 'OPD', priority: 'Standard', addedAt: Date.now() });
  ...
  if (this.page) {
    const btn = this.page
      .locator('[data-testid="add-to-queue"], button:has-text("Add to Queue")')
      .first();
...
When('I add patient to queue:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I add the patient to OPD queue', async function (this: VitoraWorld) {
  const entries = getQueue(this);
  const patientName = `${this.patient?.firstName || 'Jane'} ${this.patient?.lastName || 'Wanjiku'}`;
  const mrn = this.patient?.mrn;
  entries.push({ patientName, mrn, department: 'OPD', priority: 'Standard', addedAt: Date.now() });
...
Then('patient should be positioned according to priority', async function (this: VitoraWorld) {
  const entries = getQueue(this);
...
Then('they should be removed from the active queue', async function (this: VitoraWorld) {
...
Then('a queue ticket should be generated', async function (this: VitoraWorld) {
