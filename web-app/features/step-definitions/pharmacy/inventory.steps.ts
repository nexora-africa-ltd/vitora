...
Given('I am on the stock inventory page', async function (this: VitoraWorld) {
...
Given('I am on the pharmacy stock management page', async function (this: VitoraWorld) {
  this.currentPage = 'stock inventory';
...
Given('the following stock items exist:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I view stock levels', async function (this: VitoraWorld) {
  if (this.page) {
...
Then('the stock row should be highlighted in warning color', async function (this: VitoraWorld) {
...
Then('I should see an expiry warning', async function (this: VitoraWorld) {
  if (this.page) {
...
Given('a drug batch has expired', async function (this: VitoraWorld) {
  this.store('hasExpiredBatch', true);
});

Then('I should see an expired stock alert', async function (this: VitoraWorld) {
  if (this.page) {
...
When('I enter adjustment quantity {int}', async function (this: VitoraWorld, quantity: number) {
...
When('I confirm the adjustment', async function (this: VitoraWorld) {
  if (this.page) {
    await this.page.click('[data-testid="confirm-adjustment"]');
    await this.page.waitForResponse(
      (resp) => resp.url().includes('/api/pharmacy/stock') && resp.status() === 200
    );
  }
});

Then('the stock level should be updated', async function (this: VitoraWorld) {
  if (this.page) {
...
When('I view the inventory', async function (this: VitoraWorld) {
  this.currentPage = 'stock inventory';
  if (this.page) {
    await this.page.goto('/pharmacy/stock');
    await this.page.waitForLoadState('networkidle');
...
When('I view the stock inventory', async function (this: VitoraWorld) {
  this.currentPage = 'stock inventory';
  if (this.page) {
    await this.page.goto('/pharmacy/stock');
    await this.page.waitForLoadState('networkidle');
...
Given('stock items exist with various statuses', async function (this: VitoraWorld) {
...
When('I filter by status {string}', async function (this: VitoraWorld, status: string) {
...
Then('the batch should not be available for dispensing', async function (this: VitoraWorld) {
  this.store('batchAvailableForDispensing', false);
  if (this.page) {
    const badge = this.page.locator(
      '[data-testid="dispensable"], [data-testid="available-for-dispensing"]'
    );
  ...
  if (this.page) {
    ...
    if (data['Adjustment Type']) {
      await this.page.selectOption(
        '[data-testid="adjustment-type"], select[name="adjustment_type"]',
        data['Adjustment Type']
      );
    }
    if (data['Quantity']) {
      await this.page.fill(
        '[data-testid="adjustment-quantity"], input[name="quantity"]',
        data['Quantity']
      );
    }
    if (data['Reason']) {
      await this.page.fill(
        '[data-testid="adjustment-reason-text"], textarea[name="reason"], input[name="reason"]',
        data['Reason']
      );
    }
    if (data['Reference Number']) {
      await this.page.fill(
        '[data-testid="reference-number"], input[name="reference_number"]',
...
Then('an audit log entry should be created', async function (this: VitoraWorld) {
...
When('I add a new batch:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('the new batch should appear in the batch list', async function (this: VitoraWorld) {
...
When('I generate a stock report', async function (this: VitoraWorld) {
  if (this.page) {
    await this.page.click('[data-testid="generate-report"]');
  }
});

Then('I should see stock levels for all drugs', async function (this: VitoraWorld) {
  if (this.page) {
...
Then('the report should show low stock items highlighted', async function (this: VitoraWorld) {
  if (this.page) {
