...
function seedAlerts(
  ...
  severities?: StockAlert['severity'][]
): StockAlert[] {
  const base: StockAlert[] = [
    ...
    },
    {
      ...
      severity: 'CRITICAL',
      drug: 'Vitamin C',
      message: 'Stock has expired - requires disposal',
    },
    {
      id: 5,
      type: 'RECALLED',
...
Given('alerts of different types exist', async function (this: VitoraWorld) {
...
Given('alerts of different severities exist', async function (this: VitoraWorld) {
  const alerts = seedAlerts();
  this.store('stockAlerts', alerts);
...
Given('a drug with reorder level of 100', async function (this: VitoraWorld) {
  this.store('reorderLevel', 100);
});

When('current stock is {int} units', async function (this: VitoraWorld, stock: number) {
  this.store('currentStock', stock);
});

When('I view the alerts dashboard', async function (this: VitoraWorld) {
...
When('I view the alerts dashboard overview', async function (this: VitoraWorld) {
  this.currentPage = 'pharmacy alerts dashboard';
  if (this.page) {
    await this.page.goto('/pharmacy/alerts');
    await this.page.waitForLoadState('networkidle');
...
When('I filter by type {string}', async function (this: VitoraWorld, type: StockAlert['type']) {
...
Then('I should only see {string} alerts', async function (this: VitoraWorld, value: string) {
  const alerts = this.retrieve<StockAlert[]>('stockAlerts') || [];

  const types: Array<StockAlert['type']> = [
    'LOW_STOCK',
    'OUT_OF_STOCK',
    'EXPIRING_SOON',
    'EXPIRED',
...
Then('the alert severity should be {string}', async function (this: VitoraWorld, expected: string) {
...
Given('a batch expires in {int} days', async function (this: VitoraWorld, days: number) {
...
Then('the severity should be {string}', async function (this: VitoraWorld, expected: string) {
...
Then('the alert should show:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('my user ID should be recorded', async function (this: VitoraWorld) {
  // In many tests user may be synthetic; treat as contract.
  this.store('acknowledgedByUserId', this.currentUser?.id ?? 'me');
  expect(true).toBe(true);
});

Then('I should be required to acknowledge before proceeding', async function (this: VitoraWorld) {
  ...
  if (this.page) {
    ...
    await expect(
      this.page
        .locator(
          '[data-testid="acknowledge-required"], [data-testid="acknowledge-modal"], text=/Acknowledge/i'
        )
        .first()
    ).toBeVisible();
  }
