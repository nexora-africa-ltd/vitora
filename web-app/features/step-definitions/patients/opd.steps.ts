...
Given('patient {string} is in the OPD queue', async function (this: VitoraWorld, name: string) {
...
Given('I am documenting an encounter', async function (this: VitoraWorld) {
  const patient = this.retrieve<{ id: number }>('currentPatient');
...
Given('patient has allergy to {string}', async function (this: VitoraWorld, allergy: string) {
  const patient = this.retrieve<{ id: number }>('currentPatient');
...
Given('patient has recorded allergies', async function (this: VitoraWorld) {
  const patient = this.retrieve<{ id: number }>('currentPatient');
...
Given('I have documented:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I start a new encounter for the patient', async function (this: VitoraWorld) {
  const patient = this.retrieve<{ id: number }>('currentPatient');
  await this.page?.goto(`/encounters/new?patient=${patient?.id}`);
});

When('I select encounter type {string}', async function (this: VitoraWorld, encounterType: string) {
  await this.page?.click('[name="encounter_type"]');
  await this.page?.click(`[role="option"]:has-text("${encounterType}")`);
});

When('I save the vitals', async function (this: VitoraWorld) {
...
When('I enter chief complaint {string}', async function (this: VitoraWorld, complaint: string) {
  await this.page?.fill('[name="chief_complaint"]', complaint);
});

When('I add allergies:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I add chronic conditions:', async function (this: VitoraWorld, dataTable: DataTable) {
  ...
  for (const condition of conditions) {
    ...
    await this.page?.fill(
      ...
      condition['since'] || condition['Since'] || ''
    );
    await this.page?.click('[data-testid="save-condition"]');
  }
});

When('I search for diagnosis {string}', async function (this: VitoraWorld, searchTerm: string) {
  await this.page?.fill('[name="diagnosis_search"]', searchTerm);
  await this.page?.waitForResponse(/icd10/);
});

When('I select {string}', async function (this: VitoraWorld, option: string) {
  await this.page?.click(`[role="option"]:has-text("${option}")`);
});

When('I mark it as primary diagnosis', async function (this: VitoraWorld) {
  await this.page?.check('[name="is_primary"]');
});

When('I add diagnoses:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I order lab tests:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I try to prescribe {string}', async function (this: VitoraWorld, medication: string) {
  await this.page?.click('[data-testid="add-medication"]');
  await this.page?.fill('[name="drug_search"]', medication);
  await this.page?.click(`[role="option"]:has-text("${medication}")`);
});

When('I schedule follow-up:', async function (this: VitoraWorld, dataTable: DataTable) {
...
When('I refer patient to {string}', async function (this: VitoraWorld, department: string) {
...
When('I add referral reason {string}', async function (this: VitoraWorld, reason: string) {
  await this.page?.fill('[name="referral_reason"]', reason);
  await this.page?.click('[data-testid="submit-referral"]');
});

When('I enter:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('a new encounter should be created', async function (this: VitoraWorld) {
  const successMessage = await this.page?.locator('.toast-success, text=Encounter created').first();
  expect(successMessage).toBeTruthy();
});

Then('encounter_date should be set to today', async function (this: VitoraWorld) {
  const today = new Date().toISOString().split('T')[0];
  const dateField = await this.page?.locator('[name="encounter_date"]').inputValue();
  expect(dateField).toBe(today);
});

Then('encounter should be linked to the patient', async function (this: VitoraWorld) {
...
Then('I should be on the encounter documentation page', async function (this: VitoraWorld) {
  const url = this.page?.url();
  expect(url).toMatch(/encounters\/\d+|encounters\/new/);
});

Then('all vitals should be recorded in the encounter', async function (this: VitoraWorld) {
  const enteredVitals = this.retrieve<Array<{ vital: string; value: string }> | undefined>(
    'enteredVitals'
  );
...
Then('validation should show {string}', async function (this: VitoraWorld, result: string) {
  if (result === 'valid') {
...
Then('a critical alert should display:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('the alert should be red and prominent', async function (this: VitoraWorld) {
...
Then('the alert should persist until acknowledged', async function (this: VitoraWorld) {
  const alert = this.page?.locator('[data-alert-type="critical"]');
...
Then('I should see ICD-10 options:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('the diagnosis should be added to the encounter', async function (this: VitoraWorld) {
...
Then('a prescription should be created', async function (this: VitoraWorld) {
  const prescription = this.page?.locator('[data-testid="prescription-summary"]');
  await expect(prescription!).toBeVisible();
});

Then('it should be linked to this encounter', async function (this: VitoraWorld) {
...
Then('it should appear in pharmacy queue', async function (this: VitoraWorld) {
...
Then('I should see allergy alert:', async function (this: VitoraWorld, dataTable: DataTable) {
  const alerts = safeHashes(dataTable.hashes());
...
Then('prescription should be blocked until acknowledged', async function (this: VitoraWorld) {
...
Then('the encounter should be marked complete', async function (this: VitoraWorld) {
  const status = this.page?.locator('[data-testid="encounter-status"]');
  await expect(status!).toContainText(/complete/i);
});

Then('billing items should be generated', async function (this: VitoraWorld) {
  // Check billing summary appears
  const billing = this.page?.locator('[data-testid="billing-items"]');
  await expect(billing!).toBeVisible();
});

Then('patient should be directed to next department', async function (this: VitoraWorld) {
...
Then('reception should be notified', async function (this: VitoraWorld) {
...
Then('allergies should be displayed in a prominent banner', async function (this: VitoraWorld) {
  const banner = this.page?.locator('[data-testid="allergy-banner"]');
  await expect(banner!).toBeVisible();
});

Then('allergies should be visible throughout documentation', async function (this: VitoraWorld) {
