...
Given('I am on the patient search page', async function (this: VitoraWorld) {
  await this.page?.goto('/patients');
  await this.page?.waitForSelector('[data-testid="patient-search"], input[type="search"]');
});

Given('patients exist:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Given('patients exist in the system', async function (this: VitoraWorld) {
...
Given('a patient is marked as sensitive', async function (this: VitoraWorld) {
...
When('I click on the patient to view details', async function (this: VitoraWorld) {
...
When('I view patient journey', async function (this: VitoraWorld) {
...
Then('I should see exactly {int} result(s)', async function (this: VitoraWorld, count: number) {
  const results = await this.page?.locator('[data-testid="patient-row"], .patient-card').count();
  expect(results).toBe(count);
});

Then('I should see the matching patient', async function (this: VitoraWorld) {
  const results = await this.page?.locator('[data-testid="patient-row"], .patient-card').count();
...
Then('I should see {string} in results', async function (this: VitoraWorld, patientName: string) {
  const patientElement = this.page?.locator(
    `[data-testid="patient-row"]:has-text("${patientName}")`
  );
  await expect(patientElement!).toBeVisible();
});
...
Then('the MRN should be highlighted in the result', async function (this: VitoraWorld) {
  const searchTerm = this.retrieve<string>('searchTerm');
  const highlighted = this.page?.locator(
    `mark:has-text("${searchTerm}"), .highlight:has-text("${searchTerm}")`
  );
  await expect(highlighted!).toBeVisible();
});

Then('each result should display:', async function (this: VitoraWorld, dataTable: DataTable) {
  const fields = dataTable.raw().flat();
  const firstResult = this.page?.locator(
    '[data-testid="patient-row"]:first-child, .patient-card:first-child'
  );

  for (const field of fields) {
    const fieldElement = firstResult?.locator(
      `[data-field="${field.toLowerCase().replace(/\s+/g, '-')}"]`
    );
...
Then('I should see all {int} encounters listed', async function (this: VitoraWorld, count: number) {
  const encounters = await this.page?.locator('[data-testid="encounter-item"]').count();
  expect(encounters).toBe(count);
});

Then('encounters should be ordered by date \\(newest first\\)', async function (this: VitoraWorld) {
...
Then('the patient should NOT appear in results', async function (this: VitoraWorld) {
  const sensitivePatient = this.retrieve<{ mrn: string }>('sensitivePatient');
  const patientRow = this.page?.locator(
    `[data-testid="patient-row"]:has-text("${sensitivePatient?.mrn}")`
  );
  await expect(patientRow!).not.toBeVisible();
});

Then('the patient should appear in results', async function (this: VitoraWorld) {
  const results = await this.page?.locator('[data-testid="patient-row"], .patient-card').count();
  expect(results).toBeGreaterThanOrEqual(1);
});

Then('a {string} badge should be displayed', async function (this: VitoraWorld, badge: string) {
  const badgeElement = this.page?.locator(
    `[data-badge="${badge.toLowerCase()}"], .badge:has-text("${badge}")`
  );
  ...
  async function (this: VitoraWorld, dataTable: DataTable) {
    ...
    for (const action of actions) {
      ...
      const button = this.page?.locator(
        `button:has-text("${actionName}"), [data-action="${actionName.toLowerCase().replace(/\s+/g, '-')}"]`
      );
...
Then('I should be taken to encounter creation page', async function (this: VitoraWorld) {
  await this.page?.waitForURL(/encounters\/new|encounters\/create/);
});

Then('the patient should be pre-selected', async function (this: VitoraWorld) {
...
Then('I should see {string}', async function (this: VitoraWorld, text: string) {
  const element = this.page?.locator(`text=${text}`);
  await expect(element!).toBeVisible();
});

Then('I should see option to {string}', async function (this: VitoraWorld, action: string) {
  const button = this.page?.locator(`button:has-text("${action}"), a:has-text("${action}")`);
  await expect(button!).toBeVisible();
});

Then('only locally synced patients should appear', async function (this: VitoraWorld) {
