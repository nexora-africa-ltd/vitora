...
Given('patient is admitted', async function (this: VitoraWorld) {
  this.store('isAdmitted', true);
  this.store('patientStatus', 'Admitted');
});

Then('I should see options:', async function (this: VitoraWorld, dataTable: DataTable) {
...
Then('daily bed charge should be {string}', async function (this: VitoraWorld, rate: string) {
  this.store('dailyBedCharge', rate);
  expect(rate).toMatch(/KES\s*[0-9,]+/);
});

Then('I can update care plan', async function (this: VitoraWorld) {
