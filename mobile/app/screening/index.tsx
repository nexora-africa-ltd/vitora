...
export default function ScreeningListScreen() {
  ...
  if (screeningsQuery.isLoading) {
    return (
      <ScreenContainer>
        <LoadingState message="Loading field screenings..." />
      </ScreenContainer>
    );
  }
  ...
  return (
    <ScreenContainer>
      ...
      <SectionCard title="Screening log" subtitle={`${records.length} field records stored on this device.`}>
        {records.length === 0 ? (
          <EmptyState title="No field screenings yet" description="Start a screening while online or offline and it will be stored on the device." />
        ) : (
          records.map((record) => (
