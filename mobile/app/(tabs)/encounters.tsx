...
export default function EncountersScreen() {
  ...
  const refreshKeys = useMemo(() => [['local-encounters'], ['dashboard-summary']] as const, []);
  ...
  return (
    <ScreenList
      ...
      data={encounters}
      emptyDescription="Once clinicians create encounters from the backend, they will appear here."
      ...
      header={
        <>
          ...
          <SectionCard title="Encounters" subtitle="This view is now local-first and keeps queued offline visits visible until they reach the server.">
            <Text style={styles.sectionHelper}>Pull down to refresh after sync completes.</Text>
          </SectionCard>
        </>
      }
      keyExtractor={(encounter) => String(encounter.id)}
      onRefresh={() => void refresh()}
      refreshing={isRefreshing || encountersQuery.isRefetching}
