...
- **Billing Module**: Invoices, payments, M-Pesa integration, receipts, credit notes, payment points
...
- **Quality**: Quality improvement measures and tracking
- **Referrals**: Inter-facility referral management
- **CDS**: Clinical Decision Support rules engine
- **Scheduling**: Appointments, resources, time slots, assignment rules
- **AI / TibaBot**: Clinical chat, ICD-10 suggest, care plans, lab interpretation, discharge assessment, ICU risk, clerking autocomplete, condition prediction, clinical document generation, stored AI results with fallback services
- **Security Infrastructure**: MFA (TOTP + backup codes), emergency access override, PKI & digital signatures (RSA-2048), tamper-resistant audit log (SHA-256 hash chain), SHR document sharing
- **Interoperability**: FHIR R4 endpoints & profiles, FHIR client & validator, HL7v2 ADT service, SNOMED CT (108 concepts), code system mapping
- **Platform**: Feature flags, idempotency keys, activity feed, frontend event tracking, 6 WebSocket consumers, 18 signal modules, 9 Celery task modules
- **Emergency Dept**: Zones, bed board, real-time queue
...
class UserCertificate(models.Model):
    """X.509 certificate issued to clinicians for document signing."""
...
class CodeSystem(models.Model):
    """Terminology code system registry."""
...
- **PKI & Digital Signatures**: Certificate Authority, user certificates, RSA-2048 document signing
- **Audit Integrity**: SHA-256 hash chain verification for tamper-resistant audit logs
...
### 14.1 Current AI Capabilities (TibaBot Integration — Complete ✅)

Vitora integrates with **TibaBot**, an external clinical AI service, providing 20+ endpoints:

| Capability | Endpoint | Status |
|------------|----------|--------|
| Clinical Chat | `/api/ai/chat/` | ✅ Complete |
| ICD-10 Suggestion | `/api/ai/icd10-suggest/` | ✅ Complete |
| Care Plan Generation | `/api/ai/care-plan/generate/` | ✅ Complete |
| Care Plan (FHIR R4) | `/api/ai/care-plan/generate-fhir/` | ✅ Complete |
| CDS Evaluation | `/api/ai/cds/evaluate/` | ✅ Complete |
| Lab Interpretation | `/api/ai/lab/interpret/` | ✅ Complete |
| Discharge Assessment | `/api/ai/discharge/assess/` | ✅ Complete |
| ICU Risk Prediction | `/api/ai/icu/predict/` | ✅ Complete |
| Condition Prediction | `/api/ai/condition/predict/` | ✅ Complete |
| Clerking Autocomplete | `/api/ai/clerking/autocomplete/` | ✅ Complete |
| Clinical Document Gen | `/api/ai/clinical-document/generate/` | ✅ Complete |
| Auto-populate Forms | `/api/ai/autopopulate/` | ✅ Complete |
| AI Feedback | `/api/ai/feedback/` | ✅ Complete |
...
- **Fallbacks**: Graceful degradation when AI is unavailable (already implemented)
