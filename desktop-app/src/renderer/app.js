...
// BMI Calculation & Vital Status Functions
 ...
 * @param {object} vitals - Object with vital values
 * @returns {Array} Array of critical vital objects with name and value
 */
...
/**
 * Update vital input field with status styling
 * @param {HTMLElement} input - Input element
 * @param {string} vitalName - Vital type name
 */
function updateVitalInputStatus(input, vitalName) {
  const value = parseFloat(input.value);
  const status = getVitalStatus(vitalName, value);
  const cssClass = getVitalCssClass(status);

  // Remove existing vital status classes
  input.classList.remove('vital-normal', 'vital-warning', 'vital-critical');

  // Add new status class if value is present
  if (!isNaN(value) && cssClass) {
    input.classList.add(cssClass);
  }

  // Update status indicator if present
  const indicator = input.parentElement.querySelector('.vital-status-indicator');
  if (indicator) {
    indicator.className = 'vital-status-indicator';
    if (!isNaN(value) && status !== 'normal') {
      indicator.classList.add(`indicator-${status}`);
      indicator.textContent = status === 'critical' ? '⚠️' : '⚡';
...
/**
 * Update blood pressure field with status styling
 * @param {HTMLElement} input - Blood pressure input element
 */
function updateBloodPressureStatus(input) {
  const bp = parseBloodPressure(input.value);
  ...
  if (bp) {
    ...
    const cssClass = getVitalCssClass(overallStatus);
    if (cssClass) {
      input.classList.add(cssClass);
    }

    // Update MAP display
    updateMAPDisplay(bp.systolic, bp.diastolic);
    ...
    if (indicator) {
      ...
      if (overallStatus !== 'normal') {
        indicator.classList.add(`indicator-${overallStatus}`);
        indicator.textContent = overallStatus === 'critical' ? '⚠️' : '⚡';
        indicator.style.display = 'inline';
      } else {
        indicator.style.display = 'none';
      }
...
/**
 * Update MAP display from blood pressure values
 */
function updateMAPDisplay(systolic, diastolic) {
  const mapDisplay = document.getElementById('map-display');
  const mapValue = document.getElementById('map-value');

  if (!mapDisplay || !mapValue) return;
 ...
 */
function updateCriticalAlertsBanner() {
  const alertsBanner = document.getElementById('vitals-critical-alerts');
  const alertsList = document.getElementById('critical-alerts-list');

  if (!alertsBanner || !alertsList) return;

  const vitals = {
    temperature: parseFloat(document.getElementById('temperature')?.value),
    pulse: parseFloat(document.getElementById('pulse')?.value),
    respiratory_rate: parseFloat(document.getElementById('respiratory-rate')?.value),
    spo2: parseFloat(document.getElementById('spo2')?.value),
    blood_pressure: document.getElementById('blood-pressure')?.value,
  };

  const criticalVitals = getCriticalVitalsList(vitals);

  if (criticalVitals.length > 0) {
    alertsList.innerHTML = criticalVitals
      .map(v => `<span class="critical-vital-item">${v.label}: ${v.value}${v.unit}</span>`)
      .join(', ');
...
/**
 * Calculate and display BMI with category badge
 */
function calculateBMI() {
  ...
  if (bmi !== null) {
    const category = getBMICategoryObj(bmi);

    bmiValue.textContent = bmi;

    if (category) {
      bmiCategory.textContent = category.label;
      bmiCategory.className = 'bmi-category ' + category.cssClass;
    } else {
...
function getBMICategory(bmi) {
  const category = getBMICategoryObj(parseFloat(bmi));
  return category ? category.label : '';
}

function getBMICategoryClass(bmi) {
  const category = getBMICategoryObj(parseFloat(bmi));
  return category ? category.cssClass : '';
}

// Setup vital input event listeners for real-time status updates
function setupVitalInputListeners() {
  const vitalInputs = [
    { id: 'temperature', name: 'temperature' },
    { id: 'pulse', name: 'pulse' },
    { id: 'respiratory-rate', name: 'respiratory_rate' },
    { id: 'spo2', name: 'spo2' },
  ];

  vitalInputs.forEach(({ id, name }) => {
    const input = document.getElementById(id);
    if (input) {
      input.addEventListener('blur', () => {
        updateVitalInputStatus(input, name);
        updateCriticalAlertsBanner();
      });
  ...
  const bpInput = document.getElementById('blood-pressure');
  if (bpInput) {
    bpInput.addEventListener('blur', () => {
      updateBloodPressureStatus(bpInput);
      updateCriticalAlertsBanner();
    });
...
// Initialize vital input listeners when DOM is ready
document.addEventListener('DOMContentLoaded', setupVitalInputListeners);
